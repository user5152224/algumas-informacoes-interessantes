...
\subsection{Exist\^encia de conjuntos}
...
\[
  \varsigma(\mathcal{R})\a \mathcal{R}\in\mathcal{R}\quad\hbox{se, e somente se,}\quad\varsigma(\mathcal{R})\a \mathcal{R}\notin\mathcal{R},
\]
...
\begin{Def}
  Sejam $\sum a_n$ uma s\'erie convergente de termos estritamente positivos, e $K\subset\omega$. Seja $\kappa_K$ a caracter\'istica de $K$. Ent\~ao definimos
  \[
    \Sigma_K=\sum_{i=0}^\infty\kappa_K(i)a_i.
  \]
  Evidentemente $\Sigma_K$ \'e finito pois $\sum a_n<\infty$.
...
\begin{lem}
  Considere $\sum a_n$ como na \rf{Defini\c c\~ao}{SomaEmConjunto}. Sejam $K,L\in 2^\omega$.
  \begin{enumerate}[label = {\rm\Roman*.}]
    \item{%
      Se $K\cap L=\emptyset$, ent\~ao
      \[
        \Sigma_K+\Sigma_L=\Sigma_{K\cup L}.
      \]
    }
    \item{%
      Se $K\subset L$, ent\~ao $\Sigma_K\leq\Sigma_L$; 
    }
  \end{enumerate}
...
\begin{teo}[4.31 Remark: {\it Principles of Mathematical Analysis}]
  Seja $E\subset\;]a,b[$ enumer\'avel, virtualmente denso. Enumeremos $E$, digamos, por uma sequ\^encia $\sigma\in E^\omega$, neste caso a sequ\^encia \'e uma bije\c c\~ao entre $\omega$ e $E$. Seja tamb\'em $\sum a_n$ uma s\'erie convergente de termos estritamente positivos. Seguidamente seja $x\in\;]a,b[$ definamos
  \[
    K_x=\sigma^{-1}(]a,x[).
  \]
  Seja 
  \[
    f=\bigl\{z:\exists x\bigl(x\in\;]a,b[\;\a z=(x,\Sigma_{K_x})\bigr)\bigr\}
  \]
  Ent\~ao $f\in\R^{]a,b[}$, tem as seguintes propriedades:
  \begin{enumerate}[label = (\alph*)]
    \item{%
  ...
  \end{enumerate}
\end{teo}
\prova Primeiramente, suponha que $a<x<y<b$. Evidentemente $K_x\subset K_y$, por conseguinte do \rf{Lema}{PropSomaEmConjunto} $f(x)\leq f(y)$, segue-se (a).

  Seguidamente, provemos que para todo $x\in\;]a,b[$
  \[
    f(x)=\sup\bigl\{f(t):t\in\;]a,x[\bigr\}=f(x-).
    \label{eq2412251821}
  \]
  Para tanto seja $\varepsilon>0$, existe naturalmente $i_\varepsilon\in\omega$, tal que
  \[
    0\leq\sum_{i=0}^{i_\varepsilon-1+n}\kappa_{K_x}(i)a_i-\sum_{i=0}^{i_\varepsilon-1}\kappa_{K_x}(i)a_i<\varepsilon.
    \label{eq2412251816}
  \]
  para todo $n\in\omega$. Podemos escolher $t\in\;]a,x[$, tal que $i_\varepsilon\cap K_x\subset K_t$, temos que  $f(t)\leq f(x)$ e 
  \[
    \sum_{i=0}^{i_\varepsilon-1}\kappa_{K_x}(i)a_i\leq\sum_{i=0}^\infty\kappa_{K_t}(i)a_i=f(t).
    \label{eq2412251817}
  \]
  Tomando o limite em (\ref{eq2412251816}) e comparando com (\ref{eq2412251817}) obtemos
  \[
  ...
  \[
    I_x=\Cap\bigl\{K_t:t\in\;]x,b[\bigr\}\neq\emptyset
    \label{eq2412251901}
  \]
  se, e somente se, existe $n\in\omega$, tal que $x=\sigma_n$. Com efeito, como $I_x\subset\mathfrak{I}\sigma$, se $I_x\neq\emptyset$, existe $n\in\omega$, tal que $\sigma_n\in I_x$. Da\'i segue que $\sigma_n\in K_t$, para todo $t\in\;]x,b[$, logo, $\sigma_n\in\;]x,t[$, para todo $t\in\;]x,b[$, consequentemente $\sigma_n=x$. A rec\'iproca n\~ao \'e dif\'icil\footnote{Provar.}.

  Assim, se $x\notin{\mathfrak I}\sigma=E$, ent\~ao $I_x=\emptyset$. Suponhamos que este seja o caso. Portanto, sejam $y\in\;]x,b[$ e $\varepsilon>0$, como $\sum\kappa_{K_y}(i)a_i<\infty$, existe $i_\varepsilon$, tal que 
  \[
    \sum_{i=i_\varepsilon}^{\infty}\kappa_{K_y}(i)a_i<\varepsilon.
  \]
  Podemos escolher $t\in\;]x,y[$, tal que $i_\varepsilon^+\subset\cp K_t$. De fato, basta tomar $t<\min\{\sigma_i:i\in i_\varepsilon^+\}$. Ademais,
  \[
    0\leq f(x+)-f(x)\leq f(t)-f(x)\leq\sum_{i=i_\varepsilon}^\infty\kappa_{K_t}(i)a_i\leq\sum_{i=i_\varepsilon}^\infty\kappa_{K_y}(i)a_i<\varepsilon,
  \]
  como $\varepsilon>0$ \'e arbitr\'ario segue-se que $f(x+)=f(x)=f(-x)$, consequentemente $f$ \'e cont\'inua em $x$. Como $x\in\;]a,b[\,\cp\, E$, \'e arbitr\'ario provamos (c).

  Agora seja, $y\in\;]\sigma_n,b[$, novamente dado $\varepsilon>0$ escolhamos $i_\varepsilon>n$, tal que
  \[
    \sum_{i=i_\varepsilon}^\infty\kappa_{K_y}(i)a_i<\varepsilon.
  \]
  Escolhamos $t\in\;]\sigma_n,y[$, tal que $i_\varepsilon^+\subset\cp K_t$.
  Temos em conformidade,
  \[
    0\leq f(\sigma_n+)-f(\sigma_n)-a_n\leq\sum_{i=i_\varepsilon}^\infty\kappa_{K_t}(i)a_i\leq\sum_{i=i_\varepsilon}^\infty\kappa_{K_y}(i)a_i<\varepsilon.
  \]
