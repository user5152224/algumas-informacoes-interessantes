...
\begdoc
  ...
  \begin{Def}
    \[
      \Cup x=\bigl\{z:\exists y(y\in x\a z\in y)\bigr\}\subset.
    \]
  \end{Def}
  ...
  \begin{teo}[Defini\c c\~ao por indu\c c\~ao ou recurs\~ao]%
    Sejam $X$ um conjunto e $a\in X$. Suponha que exista uma fun\c c\~ao $F:\Cup_{p\in\omega}X^{p+1}\lra X$. Ent\~ao existe uma \'unica fun\c c\~ao $f\in X^\omega$, tal que $f(0)=a$ e $f(p+1)=F(f|_{p+1})$, para todo $p\in\omega$.
    \label{teo2101261957}
  \end{teo}
  \prova
  ...
  \[
    \begin{aligned}
      \Cup_{E\in\Gamma}E&\subset\Cup_{E\in\Gamma} X\cp g(Y\cp f(E))\cr
                        &= X\cp\Cap_{E\in\Gamma} g(Y\cp f(E))\cr
                        &= X\cp g\Bigl(\Cap_{E\in\Gamma}Y\cp f(E)\Bigr)\cr
                        &= X\cp g\Bigl(Y\cp\Cup_{E\in\Gamma}f(E)\Bigr)\cr
                        &= X\cp g\Bigl[Y\cp f\Bigl(\Cup_{E\in\Gamma}E\Bigr)\Bigr].\cr
    \end{aligned}
    \label{eq1001261141}
  \]
  Em outros termos, 
  \[
    A=\Cup_{E\in\Gamma}E=\sup\Gamma\in\Gamma
  \]  
  ...
  \begin{Def}[Opera\c c\~ao recursiva]%
    ...
    Sejam $a\in A^\omega$ e $F:\Cup_{p\in\omega}A^{p+1}\ra A$ definida por
    \[
      F(g)=g(p)\meet a_{p+1}
    \]
  ...
  \begin{axi}[Axioma da Associatividade]
    ...
    \label{axi2201262100}
  \end{axi}
  Neste ponto vale inquirir: Se $\meet$ for associativa o que dizer a respeito das seguintes opera\c c\~oes?
  \[
    \begin{aligned}
  ...
  \end{teo}
  \prova Primeiro, consideremos uma sequ\^encia $a\in A^\omega$ qualquer. Para $p=0$ temos $a_0\meet a_1$ \'e completamente n\~ao amb\'iguo. Agora, suponhamos que a propriedade seja v\'alida para todo $q\in p+1$. Estipularemos que fixados $k,l\in\omega$, tais que $k\in l+1\in p+2$, o produto dos termos de uma sequ\^encia finita $a|_{l+1\cp k}$ seja denotado por
  \[
    \Meet_{j=k}^{l}a_j.
  \]
  Esta denota\c c\~ao \'e justificada pela nossa hip\'otese de indu\c c\~ao, pois $a|_{l+1\cp k}=b|_{l-k+1}$, com $b_m=a_{m+k}$, para todo $m\in l-k+1$, $l-k+1\in p+1$ e todos as opera\c c\~oes de at\'e $q+1$ termos s\~ao univocamente determinadas, uma vez que $q\in p+1$. 

  Uma associa\c c\~ao gen\'erica $\alpha$ de elementos da sequ\^encia $a|_{p+3}$, est\'a na imagem de $\meet$. Conformemente, existem $u,v\in A$, tais que $\alpha=u\meet v$. Nestas condições, $u$ \'e a opera\c c\~ao dos termos da sequ\^encia $a|_{i+1}$, e $v$ por sua vez, \'e a opera\c c\~ao dos termos da sequ\^encia $a|_{p+3\cp i+1}$, com $i+1\in p+2$. Esta \'utima estipula\c c\~ao a respeito de $i+1$, se deve ao fato de que $v$ deve ser composto de ao menos um operando. Seguidamente, conforme nossa hip\'otese de indu\c c\~ao, a opera\c c\~ao dos termos da sequ\^encia $a|_{i+1}$ \'e
  \[
    u=\Meet_{j=0}^ia_j
  \]
  e $a|_{p+3\cp i+1}=b|_{p+2-i}$, com $b_l=a_{i+1-l}$ para todo $l\in p+3-i$. Portanto, {\it mutatis mutandis}
  \[
  ...
  \[
    \begin{aligned}
      \alpha   & = u\meet v                                                                & (\text{estipula\c c\~ao})           \cr
    ...
    \end{aligned}
  \]
  Consequentemente a propriedade \'e v\'alida para $p+1$, a prova segue do princípio de indu\c c\~ao matemática.\qed
  ...
  \begin{Def}
    ...
    \[
      X(\K)=\Cap\{n:n\in\omega\cp\{0\}\a n^*=0_\K\}.
      \label{corpo_carac}
    \]
  ...
  \proof Podemos supor que nossa fam\'ilia \'e $\{E_n:n\in\omega\}$, para cada $n$ existe uma fun\c c\~ao $f_n:\omega\rightarrow E_n$. Agora basta definirmos $f:\omega\times\omega\rightarrow U$, por $f(m,n)=f_m(n)$, com $U=\Cup_{n\in\omega}E_n$. Como $f\circ\phi^{-1}:\omega\rightarrow U$ ($\phi$ dada no teorema anterior) \'e sobrejetiva e $U$ infinito, ent\~ao $U$ \'e cont\'avel.
  
  \theorem{}{\sl%
    Seja $\mathscr A$ uma cole\c c\~ao no m\'aximo cont\'avel de membros no m\'aximo cont\'aveis. Ent\~ao $\Cup\mathscr A$ \'e no m\'aximo cont\'avel.
  }
  
  \proof Das hip\'oteses existe $I\in\omega\cup\{\omega\}$ e uma bije\c c\~ao $\kappa:I\rightarrow\mathscr A$, digamos que $A_i=\kappa(i)$. Para cada $i\in I$, existe uma bije\c c\~ao $f_i:B_i\rightarrow A_i$, com $B_i\in\omega\cup\{\omega\}$. Definamos $f\subseteq(\omega\times\omega)\times A$, com $A=\Cup\mathscr A$ por
  $$
    ((m,n),a)\in f\leftrightarrow a=f_m(n)
  $$
  Ent\~ao $f$ \'e uma fun\c c\~ao. De fato, suponhamos que $((m,n),a),((m,n),b)\in f$, ent\~ao da defini\c c\~ao $a=f_m(n)=b$. Assim, $f:\hbox{dom}\,f\rightarrow A$, \'e uma sobreje\c c\~ao. Em verdade, dado $a\in A$, existe $i\in I$, tal que $a\in A_i$, como $f_i$ \'e uma bije\c c\~ao existe $m\in B_i$, tal que $f_i(m)=a$, consequentemente $f(i,m)=a$. Usando o axioma da escolha podemos construir um conjunto $C\subseteq\omega\times\omega$ tal que $f|_C:C\rightarrow A$ \'e uma bije\c c\~ao. Como todo conjunto de $\omega\times\omega$ \'e no m\'aximo cont\'avel decorrer\'a por maior raz\~ao que $A=\Cup\mathscr A$ tamb\'em o ser\'a.
  ...
  \begin{obs}[Teorema 3.54 p. 77 do \pma]\rm%
    ...
    $$
      \omega\setminus\{0\}=\omega\cap\Cup_{n\in\omega}(k_n,k_{n+1}],
    $$
  ...
  \]
  Da mesma forma, seja $K_{n+1}$ uma das bisse\c c\~oes de $K_n$, tal que $X_{\delta_{n+1}}\subset K_{n+1}$. Destarte, constru\'imos por recurs\~ao matem\'atica uma fam\'ilia enumer\'avel $\{K_n:n\in\omega\}$, ou melhor, uma sequ\^encia $\{K_n\}$, tal que $K_{n+1}\subset K_n$ e $X_{\delta_n}\subset K_n$, para todo $n\in\omega$. Pelo teorema dos intervalos encaixados ({\it Nested Intevals Theorem}) existe $x\in\Cap_{n\in\omega}K_n$. Da constru\c c\~ao de $\{K_n\}$, temos que 
  \[
  ...
  \axiom{}{\sl%
    ...
    \begin{enumerate}[label = \Roman*.]
      ...
      \item{%
        ...
        $$
          \mu\biggl(\Cup_{i\in n+1}S_i\biggr)=\sum_{i=0}^n\mu(S_i);
        $$
      }
      \item{%
