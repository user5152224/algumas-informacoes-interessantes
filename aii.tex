...
\begdoc
  ...
  \[
    \Gamma=\bigl\{E:E\in2^X\,\a\,E\subset X\cp g\bigl(Y\cp f(E)\bigr)\bigr\}.
  \]
  Note que $\emptyset\in\Gamma$, portanto $\Gamma\neq\emptyset$. Agora seja $E\in\Gamma$, temos naturalmente
  \[
    f(E)\subset f\bigl[X\cp g(Y\cp f(E))\bigr],
  \]
  ...
  \[
    \begin{aligned}
      \Cup_{E\in\Gamma}E&\subset\Cup_{E\in\Gamma} X\cp g(Y\cp f(E))\cr
                        &= X\cp\Cap_{E\in\Gamma} g\bigl(Y\cp f(E)\bigr)\cr
  ...
  \[
    M\subset X\cp g\bigl(Y\cp f(M)\bigr)\in\Gamma,
  \]
  consequentemente,
  \[
    M= X\cp g\bigl(Y\cp f(M)\bigr),
    \label{eq0901261938}
  \]
  ...
  Segue o teorema que tem o \rf{Teorema}{teo0801261705} como corol\'ario.
  ...
  \begin{teo}[Teorema do ponto fixo para reticulados\footnote{Enunciarei o teorema sob uma perspectiva mais geral seguindo o artigo de Tarski, que generalizou o teorema aplicado ao conjunto das partes de um conjunto arbitr\'ario para reticulados completos.}]%
    ...
    \begin{enumerate}[label = ({\rm\roman*})]
      \item{%
        ${\frak A}=(A,\leq)$ um reticulado completo;
      }
      \item{%
        $f:A\ra A$ uma fun\c c\~ao crescente; 
      }
      \item{%
        $F$ o conjunto de todos pontos fixos de $f$.
      }
    \end{enumerate}
    Ent\~ao $F\neq\emptyset$ e o sistema ${\frak S}=(F,\leq)$ \'e um reticulado completo. Em particular temos
    \[
      \Vee F=\Vee\{x:x\in A\,\a\,f(x)\leq x\}
    \]
    e
    \[
      \Wedge F=\Wedge\{x:x\in A\,\a\,x\leq f(x)\}.
    \]
  \end{teo}
  \prova Provar.\qed
