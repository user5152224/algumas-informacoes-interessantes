...
\begdoc
  ...
  \begin{teo}[Exerc\'icio - \ct{HalmosNaive}]
  ...
  Seguidamente, consideremos uma cadeia arbitr\'aria ${\scr C}$ n\~ao vazia de elementos de $\Gamma$ ordenada segundo a inclus\~ao $\subset$. Conformente,
  \[
    \begin{aligned}
      \Cup_{E\in{\scr C}}E&\subset\Cup_{E\in{\scr C}} X\cp g(Y\cp f(E))\cr
                          &= X\cp\Cap_{E\in{\scr C}} g\bigl(Y\cp f(E)\bigr)\cr
                          &= X\cp g\Bigl(\Cap_{E\in{\scr C}}Y\cp f(E)\Bigr)\cr
                          &= X\cp g\Bigl(Y\cp\Cup_{E\in{\scr C}}f(E)\Bigr)\cr
                          &= X\cp g\Bigl[Y\cp f\Bigl(\Cup_{E\in{\scr C}}E\Bigr)\Bigr].\cr
    \end{aligned}
  \]
  Em outros termos, $S=\bigcup_{E\in{\scr C}}E\in\Gamma$ \'e uma cota superior de $\scr C$. Em vista de $\scr C$ ser arbitr\'aria, segue pelo lema de Zorn que $\Gamma$ possui elemento maximal $M$ segundo $\subset$. Conforme vimos em (\ref{eq0901261959}), se $M\in\Gamma$, ent\~ao 
  \[
  ...
  \[
    ...
    \label{eq0901261938}
  \]
  pois, $M$ \'e maximal. Finalmente, tomando $A=M$, segue de (\ref{eq0901261938}) que
  \[
    X\cp A=g\bigl(Y\cp f(A)\bigr).
  \]
  \qed

  A posteriori, eu pus esta demonstra\c c\~ao no llm {\bf ChatGPT}, e ele me enfatizou que a demonstra\c c\~ao do teorema anterior possui no seu cerne uma rela\c c\~ao com o teorema do ponto fixo de Knaster-Tarski. Enunciarei o teorema sob uma perspectiva mais geral seguindo o artigo de Tarski, que generalizou o teorema aplicado ao conjunto das partes de um conjunto arbitr\'ario para reticulados completos.

  \begin{teo}[Teorema do ponto fixo para reticulados\footnote{Latice-Theoretical fixpoint theorem.}]%
    Sejam 
    \begin{enumerate}[label = ({\rm\roman*})]
      \item{%
        ${\frak A}=\lge A,\leq\rge$ um reticulado completo;
      }
      \item{%
        $f\in A^A$ uma fun\c c\~ao crescente; 
      }
      \item{%
        $F$ o conjunto de todos pontos fixos de $f$.
      }
    \end{enumerate}
    Ent\~ao $F\neq\emptyset$ e o sistema $\lge F,\leq\rge$ \'e um reticulado completo. Em particular temos
    \[
      \Vee F=\Vee\{\}
    \]
  \end{teo}
