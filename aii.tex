...
\[
  \bigg|{I_f(c+h)-I_f(c)\over h}-f(c)\bigg|=\bigg|{1\over h}\int^{c+h}_cf-f(c)\,d\iota\bigg|\leq\varepsilon,
\]
...
\begin{teo}
  Em $\mathscr{D}$, podemos considerar a rela\c c\~ao $\Delta\subset\mathscr{D}\times\mathscr{D}$ definida por 
  \[
    (f,g)\in\Delta \llra f'=g'.
  \]
  Desta forma, $\Delta$ \'e uma rela\c c\~ao de equival\^encia. Ademais dado $f\in\mathscr{D}$ a classe de equival\^encia determinada por $f$ \'e 
  \[
