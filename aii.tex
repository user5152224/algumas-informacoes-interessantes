...
\begdoc
  ...
  \begin{teo}[Exerc\'icio - \ct{HalmosNaive}]
    Sejam $f:X\rightarrow Y$ e $g:Y\rightarrow X$. Ent\~ao existem subconjuntos $A\subset X$ e $B\subset Y$, tais que $f(A)=B$ e $X\cp A=g(Y\cp B)$.
    \label{teo0801261705}
  \end{teo}
  \prova ?\qed

  \begin{obs}\rm%
    Conjuntos da natureza
    \[
      S=\bigl\{x:x\in x\,\a\,X\cp\{x\}\subset g\bigr(Y\cp f(\{x\})\bigl)\bigr\}
    \]
    Satisfazem,
    \[
      X\cp S\subset g(Y\cp f(S)),
    \]
    mas n\~ao h\'a garantias da igualdade. Foram in\'umeras tentativas fracassadas.
  \end{obs}
