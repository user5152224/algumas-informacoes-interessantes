...
\begin{enumerate}[label = (\alph*)]
  ...
  \item{%
    Sejam $m,n\in\omega$, note que \'e suficiente notar que
    \[
      \begin{split}
        \sigma(mn)&=\sigma\bigl((-m)(-n)\bigr)\cr
                  &=-\sigma(-mn)\cr
...
Sejam $m,n\in Z$. Se um deles \'e zero digamos $n=0$, ent\~ao
\[
  \zeta(m+0)=\zeta(m)=\zeta(m)+0_K=\zeta(m)+\zeta(0)
\]

Se ambos s\~ao estritamente negativos ou ambos s\~ao estritamente positivos, em qualquer caso vale
\[
  \begin{split}
    \zeta(m+n) &=|m+n|*\sigma(m+n)\cr
               &=(|m|+|n|)*\sigma(m+n)\cr
               &=|m|*\sigma(m+n)+|n|*\sigma(m+n)\cr
               &=|m|*\sigma(m)+|n|*\sigma(n)\cr
               &=\zeta(m)+\zeta(n),\cr
  \end{split}
\]
pois $\sigma(m+n)=\sigma(m)=\sigma(n)$.

Agora, suponhamos que eles t\^em sinais contr\'arios e s\~ao n\~ao nulos. Admitamos sem perda de generalidade que $|m|\leq|n|$. Se $m+n<0$, ent\~ao $n<0<m$  
\[
  \zeta(m+n)=(-(m+n))*\sigma(m+n)
\]

\[
  \begin{split}
    \zeta(m)+\zeta(n)&=\zeta(m+n)=\zeta(-((-m)+(-n)))\cr
                     &=-\zeta(-(m-(-n)))\cr
  \end{split}
\]
\qed
