...
\def\parte#1{
  ...
  {%
    \centering
    {\fontsize{120}{120}\selectfont \expandafter\MakeUppercase{\romannumeral\the\partnum}}% 
...
\begdoc
  ...
  % TODO: Completar a demonstra\c c\~ao.
  ...
  \end{teo}
  \prova
  Primeiramente seja $p\in\omega$, definamos
  \[
    \mathscr{F}_p=\bigl\{g:g\in X^{p+2}\,\a\,g(0)=a\,\a\,\forall q\bigl(q\in p+1\lra g(q+1)=F(g|_{q+1})\bigr\}
  \]
  \[
    C=\Bigl\{p:p\in\omega\,\a\,\exists! g\Bigl(g\in X^{p+2}\,\a\,g(0)=a\,\a\,\forall q\bigl(q\in p+1\lra g(q+1)=F(g|_{q+1})\bigr)\Bigr)\Bigr\}.
  \]
  Primeiro, provemos que $0\in C$. 




  Com efeito, seja $h\in X^1$, tal que $h(0)=a$, basta definir $g|(1)=h$ e $g(1)=F(g|(1))$, \'e evidente que $g\in X^2$, \'e tal que $g(0)=a$ e $g|(2)=(Fg|)|(2)$; al\'em disso $g$ \'e \'unica, pois $\gamma(0)=a$ e $\gamma|(2)=(F\gamma|)|(2)$, temos que $g|(1)=\gamma|(1)$, logo $g(1)=F(g|(1))=F(\gamma|(1))=\gamma(1)$, o que prova a unicidade, logo $0\in C$. Suponha que $p\in C$, segue, portanto, que existe uma \'unica $h\in X^{p+2}$, tal que $h(0)=0$ e $h|(p+2)=(Fh|)|(p+2)$. Definamos $g|(p+2)=h|(p+2)$ e $g(p+2)=F(h|(p+2))$, segue-se $g\in X^{p+3}$. Se $\gamma\in X^{p+3}$, \'e tal que $\gamma(0)=a$ e $\gamma|(p+3)=(F\gamma|)|(p+3)$, ent\~ao $\gamma|(p+2)=h|(p+2)$, pois $h$ \'e \'unica. Como consequ\^encia $g(p+2)=F(h|(p+2))=F(\gamma|(p+2))=\gamma(p+2)$, o que prova a unicidade de $g$, acarretando que $p+1\in C$. Pelo princ\'ipio de indu\c c\~ao matem\'atica $C=\omega$. 

  Para cada $p$ seja $g_p\in X^{p+2}$ a \'unica fun\c c\~ao tal que $g_p(0)=a$ e $g_p|(p+2)=(Fg|)|(p+2)$, defina $f=\bigcup_{p\in\omega}g_p$. Provemos que $f$ \'e uma fun\c c\~ao. De fato, suponhamos que $(a,b),(a,c)\in f$, existem portanto $m,n\in\omega$, que podemos supor sem perda de generalidade, tais que $m\leq n$, $(a,b)\in g_m$ e $(a,c)\in g_n$, como $g_m|(m+2)=g_n|(m+2)$ pois $g_m$ \'e \'unica, segue-se necessariamente que $b=g_m(a)=g_n(a)=c$, provando portanto que $f$ \'e uma fun\c c\~ao. Ademais, $f$ \'e tal que $f(0)=a$ e $f(p+1)=F(f|(p+1))$; al\'em disso, seja $\phi$ uma outra fun\c c\~ao cumprindo as propriedades estipuladas, naturalmente para todo $p\in\omega$ vale 
  \[
    f|(p+2)=g_p|(p+2)=\phi|(p+2),
  \]
  portanto, $f=\phi$, i.e., f \'e a \'unica com as propriedades estipuladas.\qed
  ...
  O objetivo das pr\'oximas se\c c\~oes \'e aplicar transforma\c c\~oes ao sistema de coordenadas, de tal maneira que a c\^onica seja facilmente reconhecida. Doravante, a menos da men\c c\~ao expl\'icita em contr\'ario, sempre quando referirmos a $g$, estaremos considerando um polin\^omio como estipulado na \rf{Defini\c c\~ao}{def3012252158}.

  \begin{teo}[Exerc\'icio 23-10 {[Boulos e Camargo]}]
  \end{teo}
  \prova Primeiramente consideremos o sistema

  % TODO: Consertar esta matriz.
  \[
    \left\{
    \begin{aligned}
               ax +{b\over2}y + {d\over2} &=0\cr
      {b\over 2}x +cy         + {e\over2} &=0\cr
    \end{aligned}
    \right.
    \label{S291220252034}
  \]

  Sejam $(h,k)$ e $(u,v)$ solu\c c\~oes de do sistema (\ref{S291220252034}). Em verdade n\~ao \'e dif\'icil notar que $(h+k,u+v)$ tamb\'em o \'e, bastando para isto substituir as respectivas coordenadas no sistema (\ref{S291220252034}) e atestar as igualdades.
 
  Notemos em seguida que qualquer solu\c c\~ao $(x,y)$ de (\ref{S291220252034}), satisfaz
  \[
    ax^2+bxy+cy^2+{d\over2}x+{e\over2}y=x(ax+{b\over2}y+{d\over2})+y({b\over2}x+cy+{e\over2})=0;
    \label{eq2912252053}
  \]
  al\'em disso, para uma tal solu\c c\~ao $(x,y)$, vale
  \[
    g(x,y)={d\over2}x+{e\over2}y+f.
    \label{eq2912252123}
  \]
  ...
  \[
    ...
    \end{split}
  \]
  Em outros termos $g$ \'e constante nas solu\c c\~oes de (\ref{S291220252034}).\qed
