...
%  \]
  ...
  Consideremos um sistema de coordenadas cartesianas e um sistema cujos eixos sejam obtidos do primeiro por uma rota\c c\~ao dum \^angulo $\theta$ no setido anti-hor\'ario em rela\c c\~ao ao eixo das abscissas. 

  \begin{figure}[!h]
    \caption{Rota\c c\~ao dum sistema de coordenadas cartesiano por um \^angulo $\theta$.}
  \end{figure}

  Consideremos um ponto $w=(u,v)_\theta$ cujas coordenadas s\~ao dadas em rela\c c\~ao ao sistema rotacionado. Nosso objetivo \'e determinar as coordenadas de $w$ relativo ao primeiro sistema. A t\'itulo de exemplo, usaremos um m\'etodo geom\'etrico para um caso particular, que pode ser adaptado aos outros casos.
  ...
  % TODO: Salientar que rotações dos eixos por ângulos maiores que 90 podem ser observadas como rotações em ângulos ]0,-90[. Note o que importa são os eixos, que são em essência retas, e não a base que as determina.
  ...
  Uma outra maneira, mais geral \'e recorrer \`a \'algebra linear, munido com os recursos da geometria anal\'itica, ou mais geralmente espaços vetoriais com produto interno. Seja ${\mathcal B}=\{e_1,e_2\}$ a base can\^onica do $\R^2$. Uma base ortonormal, para o segundo sistema \'e 
  \[
  ...
  \[
    \arccos\Bigl({\lge r,e_1\rge\over |r||e_1|}\Bigr)=\arccos(\cos\theta)=\theta,
  \]
  supondo evidentemente que $\theta\in\,]0,\pi[$. Esta escolha n\~ao \'e t\~ao restritiva quanto se parece, pois fun\c c\~oes circulares s\~ao per\'iodicas de per\'iodo $2\pi$. Podemos para fins de c\'alculo reduzir os \^angulos ao intervalo $[0,2\pi]$; al\'em disso, se $\theta\in[\pi,2\pi]$, podemos considerar o outro ramo invers\'ivel de $\cos$, a saber, aquele cujo dom\'inio \'e $[-\pi,0]$. E por que n\~ao consideramos m\'ultiplos de $\pi$? A raz\~ao \'e que precipuamente, ter\'iamos os mesmo sistema de coordenadas, o que nos importa somente s\~ao os eixos, i.e., as retas correspondentes, n\~ao como as determinamos.
  ...
  \[
    ...
    \label{eq0301261607}
  \]
  Se reduzirmos nossa escolha ao intervalo $[0,2\pi[$, temos duas solu\c c\~oes para (\ref{eq0301261607}), uma no semiciclo superior e a outra no semiciclo inferior.\qed
