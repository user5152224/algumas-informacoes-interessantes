...
\begin{Def}
  Sejam $K$ um corpo com unidade $1_K$ e zero $0_K$ e
  \[
    E=\{n:n\in\omega\cp\{0\}\a n^*=0_K\}.
  \]
  Definimos a caracter\'istica de $K$ por 
  \[
    X(K)=\bigcap E.
  \]
  Observe que $X(K)$ \'e sempre um n\'umero natural, e $E=\emptyset$ se, e somente se, $X(K)=0$.
  \label{def_X}
\end{Def}
...
Sejam $m,n\in Z$. Se $m,n\geq 0$ evidentemente
\[
  \zeta(m+n)=|m+n|*\sigma(m+n)=m*\sigma(m)+n*\sigma(n)=\zeta(m)+\zeta(n),
\]
porquanto, $\sigma(m)=\sigma(n)=\sigma(m+n)$.

Agora, suponha sem perda de generalidade que $m<0$, temos que $-m\in\omega$, donde
\[
  \zeta(m+n)+\zeta(-m)=\zeta(n),
\]
logo,
\[
  \zeta(m+n)=\zeta(n)-\zeta(-m)=\zeta(m)+\zeta(n).
\]
...
Por fim, seja $n\in\ker\zeta$, temos que $\zeta(n)=n^*=0_K$. Se $n\neq 0$, podemos sempre supor que $n>0$, bastando para isso considerar $-n$. Conformemente, ter\'iamos que existe $n\in\omega\cp\{0\}$ tal que $n^*=0_K$, a fortiori $X(K)\neq0$, o que \'e uma contradi\c c\~ao. Isto implica por sua vez que $\ker\zeta=\{0\}$, confirmando que $\zeta$ \'e um isomorfismo.
\qed
