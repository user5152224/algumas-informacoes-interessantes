...
\usepackage[letterspace=147]{microtype}
...
%  {%
%    \centering
%    {\fontsize{70}{70}\selectfont \expandafter\MakeUppercase{\romannumeral\the\partnum}}% 
%            
%    \vspace{1cm}\noindent
%    {%            
...
\titleformat{\subsection}{\bfseries\footnotesize\MakeUppercase}{\thesubsection}{1em}{}[]
\titleformat{\subsubsection}{\bfseries\footnotesize\MakeUppercase}{\thesubsubsection}{1em}{}[]

% Controle dos níveis chapter (1), section, subscetion e subsubsection.
\setcounter{secnumdepth}{4}

% Exibir a numeração do nível subsubsection na tabela de conteúdos
...
\fancypagestyle{est}{%
  \def\fontface{}
  \fancyhf{}
  \fancyhead[RO]{\footnotesize\MakeUppercase{\rightmark}\quad\bfseries\thepage}
  \fancyhead[LE]{\footnotesize{\bfseries\thepage}\quad\MakeUppercase{\leftmark}}
...
\newcommand{\blankpage}{
  \begingroup
    ...
    \hbox{}\vfill
    \centering
    {\slshape P\'agina deixada intencionalmente em branco}
    \vfill\hbox{}
    \break
  \endgroup
}
...
\def\meet{\curlywedge}
\def\join{\curlyvee}
\def\Meet{\bigcurlywedge}
\def\Join{\bigcurlyvee}
\def\fr#1#2{\frac{#1}{#2}}
\def\frak{\mathfrak}
\def\K{\mathds{K}}
\def\la{\leftarrow}
...
{}
{0.5em}
{\scshape\thmname{\scshape #1} \thmnumber{\scshape #2}\thmnote{ \scshape(#3)}\ }
...
\begdoc
  ...
  \null\vfill\hfill
  \parbox[t]{11.2cm}{\it%
    Dedico o resultado destas anota\c c\~oes aos meus Senhores. Primeiro, a Deus, que, consoante o seu desígnio, me concede o discernimento e o p\~ao de cada dia. Segundo, a Jesus, n\~ao menos importante, em cujas palavras tanto me comprazo, e ciente de que Ele \'e O Caminho e A Verdade. 
  }{}
  \newpage        
  ...
  \chapter*{Prel\'udio}
    O texto que segue trata-se de um comp\^endio de assuntos que acho relevantes. Portanto, sem margem para d\'uvidas, o texto reflete minha subjetividade. Este texto estar\'a em constru\c c\~ao cont\'inua, de maneira que n\~ao haver\'a vers\~ao final enquanto as circunst\^ancias da vida sobrepujarem o meu \'impeto de colecionar tais informa\c c\~oes\footnote{E vieram \`a tona, estou abandonando este projeto. Infelizmente devo admitir, a academia, em especial as pessoas envolvidas---n\~ao todas---com seus egos que beiram o incomensur\'avel, s\~ao deidades em desistimula\c c\~ao.}. Alguns resultados s\~ao de minha autoria, mas certamente est\~ao enviesados por alguma obra, de maneira que n\~ao descobri a roda, mas me sustentei sobre o trabalho de v\'arios indiv\'iduos perspicazes. Al\'em disso, \'e imposs\'ivel colecionar tudo o que nos interessa. Destarte, me ative aos que, ao menos, tive alguma ideia de demonstra\c c\~ao. 
    \vskip .5cm\hfill Harllen Araújo de Sena
  ...
  \begin{Def}[Sistema de axiomas mutualmente independentes]
    Um sistema de axiomas \'e dito {\rm\scshape mutualmente independentes} se nenhum axioma do sistema pode ser derivado dos outros por m\'etodos de infer\^encia l\'ogica, i.e., da l\'ogica proposicional e de disciplinas precedentes\footnote{Isto est\'a em conformidade segundo Tarski. Quais seriam estas disciplinas?}.
  \end{Def}
  ...
  Decidi abordar a constru\c c\~ao dada no ap\^endice do livro {\it General Topology} de John L. Kelley ({\it vide} pp. 250--281). Conforme atestei, o sistema empregado por Kelley, \'e uma adapta\c c\~ao do sistema de A. P. Morse, confirma na referida obra.
  ...
  Enfatizo que os objetos da teoria de classes, resumem-se, como \'e esperado, \`a classes. Portanto, `$\{x:\phi(x)\}$' denota uma classe, da\'i o nome de {\it classifier} ou {\bf classificador}.
  ...
  \begin{Def}
    \[
      \bigcup x=\bigl\{z:\exists y(y\in x\a z\in y)\bigr\}\subset.
    \]
  \end{Def}
  ...
  \begin{teo}[\scshape 31 Theorem]
    \[
      \forall x,y\Bigl(x\subset y\lra \bigcup x\subset \bigcup y\a\bigcap y\subset\bigcap x\Bigr).
    \]
  \end{teo}
  \prova Sejam $x,y,z$ classes arbitr\'arias, tais que $x\subset y$. Primeiro temos
  \[
    z\in\bigcup x \lra\exists w(w\in x\a z\in w)\lra\exists w(w\in y\a z\in w)\lra z\in\bigcup y.
  \]
  Segundo,
  \[
    z\in\bigcap y \lra\forall w(w\in y\lra z\in w)\lra\exists w(w\in x\lra z\in w)\lra z\in\bigcap x.
  \]
  \qed
  \begin{teo}[\scshape 32 Theorem]
    \[
      \forall x,y\Bigl(x\in y\lra x\subset\bigcup y\a \bigcap y\subset x\Bigr)
    \]
    \label{T32}
  \end{teo}
  \prova Sejam $x,y,z$, classes aleat\'orias tais que $x\in y$. Inicialmente, temos
  \[
    z\in x\lra \exists w(w\in y\a z\in w)\lra z\in\bigcup y.
  \]
  Finalmente,
  \[
    z\in\bigcap y\lra\forall w(w\in y\lra z\in w)\lra z\in x.
  \]
  \qed
  ...
  \begin{teo}[34 Theorem]
    \[
      \emptyset = \bigcap\U\a\U=\bigcup\U.
    \]
    \label{teo34}
  \end{teo}
  \prova Primeiramente suponhamos por {\it reductio ad absurdum} que exista $x\in\bigcap\U$, ent\~ao $\varsigma(x)$, pelo \rf{Teorema}{T26} segue-se $\emptyset\subset x$; assim necessariamente $\varsigma(\emptyset)$, ou equivalentemente $\emptyset\in\U$. Pelo \rf{Teorema}{T32} tem-se que $\bigcup\U\subset\emptyset$, novamente pelo \rf{Teorema}{T26} temos $\emptyset\subset\bigcap\U$, logo, pelo \rf{Teorema}{T27} e da suposi\c c\~ao inicial decorre que $x\in\bigcap\U=\emptyset$, o que \'e uma contradi\c c\~ao. Consequentemente, n\~ao \'e o caso que exista $x\in\bigcap\U$, i.e., $\bigcap\U=\emptyset$[\footnote{Segui a mesma linha de racioc\'inio de Kelley, mas com a minhas adapta\c c\~oes.}.
  
  Agora, seja $x\in\U$, pelo \AS existe $y\in\U$, tal que $x\in y$, pois $x\subset x$, pela defini\c c\~ao de $\bigcup\U$, segues-se que $x\in \bigcup\U$, consequentemente $\U\subset\bigcup\U$. Por outro lado, pelo \rf{Teorema}{T26}, temos $\bigcup\U\subset\U$, consequentemente pelo \rf{Teorema}{T27}, $\bigcup\U=\U$.\qed
  
  \begin{teo}[35 Theorem]
    Para toda classe $x$, se $x\neq\emptyset$, ent\~ao $\varsigma(\bigcap x)$.
    \label{T35}
  \end{teo}
  \prova Se $x\neq\emptyset$, ent\~ao existe $y\in x$, logo $\varsigma(y)$ e pelo \rf{Teorema}{T32} tem-se $\bigcap x\subset y$, da\'i e do \rf{Teorema}{T32}, decorre que $\varsigma(\bigcap x)$.\qed 
  ...
  \begin{teo}[38 Theorem]
    ...
    \label{teo38}
  \end{teo}
  \prova Se $\varsigma(x)$, pelo \AS existe $y\in\U$, tal que para todo $z$, se $z\subset y$, ent\~ao $z\in y$. Como consequ\^encia, $2^x\subset y$, pelo \rf{Teorema}{T33} segue-se que $\varsigma(2^x)$.\qed
  ...
  \begin{Def}[40 Definition]%
    $\{x\}=\{y:x\in\U\lra y=x\}$.
  \end{Def}

  \begin{teo}[41 Theorem]
    Se $\varsigma(x)$, ent\~ao, para cada $y$, $y\in\{x\}$ se, e somente se, $y=x$.
  \end{teo}
  \prova Se $\varsigma(x)$, ent\~ao $x\in\U$. Logo, $y\in\{x\}$ se, e somente se, $y=x$.
  
  \begin{teo}[42 Theorem]
    Se $\varsigma(x)$, ent\~ao $\varsigma(\{x\})$.
    \label{teo42}
  \end{teo}
  \prova Pelo \rf{Teorema}{teo38} se $\varsigma(x)$, ent\~ao $\varsigma(2^x)$, como $\{x\}\subset 2^x$, segue-se pelo \rf{Teorema}{T33} que $\varsigma(\{x\})$.\qed

  \begin{teo}[43]
    $\neg\varsigma(x)$ se, e somente se, $\{x\}=\U$. 
    \label{teo43}
  \end{teo}
  \prova Se $\neg\varsigma(x)$, ent\~ao 
  \[
    (x\in\U\lra y=x)\a\varsigma(y)
  \]
  \'e uma tautologia, pois o antecedente $x\in\U$ \'e sempre falso, pelo \rf{Teorema}{teo2901261640}, decorre $\{x\}=\U$. Se $\varsigma(x)$, ent\~ao do \rf{Teorema}{teo42} $\varsigma(\{x\})$, logo $\{x\}$ n\~ao pode ser igual a $\U$, disto a rec\'iproca segue por contrapositiva.\qed

  \begin{teo}[44 Theorem]
    Se $\neg\varsigma(x)$, ent\~ao $\bigcap \{x\}=\emptyset$ e $\bigcup \{x\}=\U$. Se $\varsigma(x)$, ent\~ao $x=\bigcap \{x\}=\bigcup \{x\}$.
  \end{teo}
  \prova A primeira condicional segue do \rf{Teorema}{teo34} e do \rf{Teorema}{teo43}.


% Final do apêndice do GT por Kelley

  \newpage

  \begin{teo}[Defini\c c\~ao por indu\c c\~ao ou recurs\~ao]%
    Sejam $X$ um conjunto e $a\in X$. Suponha que exista uma fun\c c\~ao $F:\bigcup_{p\in\omega}X^{p+1}\lra X$. Ent\~ao existe uma \'unica fun\c c\~ao $f\in X^\omega$, tal que $f(0)=a$ e $f(p+1)=F(f|_{p+1})$, para todo $p\in\omega$.\qed
    \label{teo2101261957}
  \end{teo}
  \prova
  ...
  \]
  Primeiro, considere $g\in X^{2}$, tal que $g(0)=a$ e $g(1)=F(g|_1)$. Naturalmente se $\gamma\in{\mathfrak F}_0$, ent\~ao $\gamma(0)=a=g(0)$, logo, $\gamma|_1=g|_1$ e consequentemente $\gamma(1)=F(\gamma|_1)=F(g|_1)=g(1)$, o que prova que $0\in C$. Em seguida, suponhamos que $p\in\omega$, segue que existe uma \'unica $h\in{\mathfrak F}_p$, defina $g\in X^{p+3}$, tal que $g|_{p+2}=h$ e $g(p+2)=F(g|_{p+2})$. Seja agora $\gamma\in{\frak F}_{p+1}$, pela unicidade de $h$ segue-se que $\gamma|_{p+2}=h=g|_{p+2}$, por conseguinte $\gamma(p+2)=F(h)=g(p+2)$, que por sua vez, acarreta $\gamma=g$, o que prova a unicidade de $g$. Destarte, $p+1\in C$. Como $\omega$ \'e o conjunto sucessor (indutivo) minimal segue-se que $C=\omega$. 

  Em verdade, provamos que ${\frak F}_p$ \'e unit\'ario para todo $p\in\omega$. Digamos que $f_p\in{\frak F}_p$ e definamos $f=\bigcup_{p\in\omega} f_p$. Suponhamos que $(a,b),(a,c)\in f$. Naturalmente, existem $p,q\in\omega$ tais que $(a,b)\in f_p$ e $(a,c)\in f_q$, os quais sem perda de generalidade podemos supor $p\leq q$. Como $f_q|_{p+2}\in{\frak F}_p$, podemos inferir que $f_q|_{p+2}=f_p$; assim $b=f_p(a)=f_q(a)=c$, o que prova que $f\in X^\omega$. Agora seja $p\in\omega$, notemos que $f|_{p+2}=f_p$, consequentemente $f(0)=a$ e $f(p+1)=F(f|_{p+1})$, portanto $f$ satisfaz as hip\'oteses do teorema, i.e., provamos a exist\^encia. Ademais, seja $\phi\in X^\omega$ uma outra fun\c c\~ao que satisfaz as estipula\c c\~oes do teorema, temos que para todo $p\in\omega$, vale $f|_{p+2},\phi|_{p+2}\in{\frak F}_p$, por conseguinte $f|_{p+2}=f_p=\phi|_{p+2}$, que por sua vez implica que $f=\phi$, pois $p\in\omega$ \'e arbitr\'ario, provando portanto a unicidade de $f$.\qed

  O pr\'oximo teorema \'e um exerc\'icio que me apareceu quando lia \ct{HalmosNaive}, este teorema me fez estancar no texto. Eu voltei a atacar o problema v\'arios meses depois, com v\'arios fracassos em sequ\^encia. Em fim consegui, com adapta\c c\~oes posteriores.
   
  \begin{teo}[Exerc\'icio - \ct{HalmosNaive}\footnote{Na minha primeira demonstra\c c\~ao deste resultado eu fiz recorr\^encia ao \AEs, precipuamente um de seus equivalentes, a saber, o lema de Zorn. Em verdade, ele n\~ao \'e necess\'ario, pois conforme se observa basta a unicidade do supremo, que por sua vez, \'e garantida no conjunto das partes munido com a ordem parcial induzida pela inclus\~ao $\subset$. Esta observa\c c\~ao tardia s\'o veio ao meu conhecimento depois que pus a minha demonstra\c c\~ao inicial no llm {\bf ChatGPT}, ele enfatizou que a demonstra\c c\~ao do teorema possui no seu cerne uma rela\c c\~ao com o teorema do ponto fixo de Knaster-Tarski. Embora ele n\~ao tenha sugerido nenhuma adapta\c c\~ao \`a minha prova, eu percebi posteriormente que o ponto principal da demonstra\c c\~ao jaz sobre a desigualdade (\ref{eq1001261141}). Esta \'ultima usada anteriormente para provar que toda cadeia n\~ao vazia admite {\it supremum}, e consequentemente concluir a exist\^encia dum elemento maximal.}]
    Sejam $f:X\rightarrow Y$ e $g:Y\rightarrow X$. Ent\~ao existe um subconjunto $A\subset X$, tal que $X\cp A=g(Y\cp f(A))$.
    \label{teo0801261705}
  \end{teo}
  \prova 
  Defina a cole\c c\~ao
  \[
    \Gamma=\{E:E\in2^X\,\a\,E\subset X\cp g(Y\cp f(E))\}.
  \]
  Note que $\emptyset\in\Gamma$, portanto $\Gamma\neq\emptyset$. Agora seja $E\in\Gamma$, temos naturalmente
  \[
    f(E)\subset f[X\cp g(Y\cp f(E))],
  \]
  que por sua vez acarreta
  \[
    X\cp g(Y\cp f(E))\subset X\cp g\bigl[Y\cp f\bigl(X\cp g(Y\cp f(E))\bigl)\bigr].
  \]
  Podemos concluir que se $E\in\Gamma$, ent\~ao
  \[
    E\subset X\cp g(Y\cp f(E))\in\Gamma.
    \label{eq0901261959}
  \]
  ...
  \[
    \begin{aligned}
      \bigcup_{E\in\Gamma}E&\subset\bigcup_{E\in\Gamma} X\cp g(Y\cp f(E))\cr
                        &= X\cp\bigcap_{E\in\Gamma} g(Y\cp f(E))\cr
                        &= X\cp g\Bigl(\bigcap_{E\in\Gamma}Y\cp f(E)\Bigr)\cr
                        &= X\cp g\Bigl(Y\cp\bigcup_{E\in\Gamma}f(E)\Bigr)\cr
                        &= X\cp g\Bigl[Y\cp f\Bigl(\bigcup_{E\in\Gamma}E\Bigr)\Bigr].\cr
    \end{aligned}
    \label{eq1001261141}
  \]
  Em outros termos, 
  \[
    A=\bigcup_{E\in\Gamma}E=\sup\Gamma\in\Gamma
  \]  
  Conforme vimos em (\ref{eq0901261959}), segue-se que
  \[
    A\subset X\cp g(Y\cp f(A))\in\Gamma,
  \]
  consequentemente,
  \[
    A= X\cp g(Y\cp f(A)),
  \]
  pois, $A$ \'e o elemento m\'aximo de $\Gamma$.\qed

  Conforme notado por Halmos em \ct{HalmosNaive} o \rf{Teorema}{teo0801261705} \'e uma maneira elegante de se demonstrar o

  \begin{cor}[Bernstein-Cantor-Schröder]
    Sejam $X$ e $Y$ conjuntos tais que existe uma inje\c c\~ao pr\'opria de $X$ em $Y$ e vice-versa. Ent\~ao $X$ \'e equivalente a $Y$.
  \end{cor}
  \prova Pelo \rf{Teorema}{teo0801261705} existe $A\subset X$, tal que 
  \[
    X\cp A=g(Y\cp f(A)).
  \]
  Definamos $h:X\ra Y$, por
  \[
    h(x)=
    \begin{cases}
      f(x),      & x\in A\cr
      g^{-1}(x), & x\in X\cp A\cr
    \end{cases}
  \]
  Ent\~ao sabemos naturalmente que $h$ \'e uma bije\c c\~ao entre $X$ e $Y$, o que prova o requerido.\qed

  \section{Outras teorias}

  % TODO: Provar.

  \begin{Def}[Opera\c c\~ao recursiva]%
    Sejam $A$ um conjunto e $\meet:A\times A\ra A$ uma opera\c c\~ao bin\'aria. Estipularemos que 
    \[
      u\meet v=\meet(u,v),
    \]
    para todo $u,v\in A$.

    Sejam $a\in A^\omega$ e $F:\bigcup_{p\in\omega}A^{p+1}\ra A$ definida por
    \[
      F(g)=g(p)\meet a_{p+1}
    \]
    para toda $g\in A^{p+1}$ e $p\in\omega$. Do \rf{Teorema}{teo2101261957}, fixado $k\in\omega$, existe uma \'unica $f\in A^\omega$, tal que $f(0)=a_k$ e $f(p+1)=F(f|_{p+1})$, para todo $p\in\omega$. Convenientemente, definamos
    \[
      \Meet_{i=k}^{k+n} a_i=f(n)
    \]
    para todo $n\in\omega$. Em particular, se $n\in\omega$ e $k=1$, temos naturalmente
    \[
      \Meet_{i=1}^{n+1} a_i=f(n).
    \]
    Em conformidade, para $k,n\in\omega$ tamb\'em vale
    \[
      \Meet_{i=k}^{k+n+2} a_i=\Meet_{i=k}^{k+n+1} a_i\meet a_{k+n+2}.
    \]
    \label{def2101262009}
  \end{Def}

  A \rf{Defini\c c\~ao}{def2101262009} nos d\'a formalmente a defini\c c\~ao de uma opera\c c\~ao recursiva. Particularmente, se $A=\R$ e $\meet=+$ em $\R$, ent\~ao temos a somat\'oria
  \[
    \sum_{i=1}^{n+1}a_n.
  \]
  Se $\meet=\cdot$ e $A=\R$, ent\~ao temos a produt\'oria
  \[
    \prod_{i=1}^{n+1}a_n.
  \]
  Em verdade, intuitivamente temos que se $k=1$, ent\~ao
  \[
    \sum_{i=1}^2 a_i=\sum_{i=1}^1 a_i+a_2=a_1+a_2
  \]
  e
  \[
    \sum_{i=1}^3 a_i=\sum_{i=1}^2 a_i+a_3=(a_1+a_2)+a_3;
  \]
  e
  \[
    \sum_{i=1}^4 a_i=\sum_{i=1}^3 a_i+a_4=((a_1+a_2)+a_3)+a_4
  \]
  {\it ad infinitum}; {\it idem}, para qualquer opera\c c\~ao bin\'aria, {\it mutatis mutandis}.

  \begin{axi}[Axioma da Associatividade]
    Sejam $A$, $\meet$ e $a$ como na \rf{Defini\c c\~ao}{def2101262009}. Suponha que $\meet$ satisfa\c ca o seguinte axioma
    \[
      (u\meet v)\meet w=v\meet(v\meet w)
    \]
    Ent\~ao diremos que $\meet$ \'e {\bf associativa}.
    \label{axi2201262100}
  \end{axi}
  Neste ponto vale inquirir: Se $\meet$ for associativa o que dizer a respeito das seguintes associa\c c\~oes?
  \[
    \begin{aligned}
      ((u\meet v)\meet w)\meet x \cr
      (u\meet (v\meet w))\meet x \cr
      u\meet ((v\meet w)\meet x) \cr
      u\meet (v\meet (w\meet x)) \cr
      (u\meet v)\meet (w\meet x) \cr
  ...
  O pr\'oximo teorema prova que se $\meet$ \'e associativa ent\~ao todas elas ser\~ao iguais. Ademais, o mesmo vale para qualquer associa\c c\~ao composta por $p$ elementos com $p\in\omega\cp2$.

  \begin{teo}[Associatividade generalizada]%
    Considermos o contexto do \rf{Axioma}{axi2201262100}. Ent\~ao para cada $p\in\omega$ o resultado de associa\c c\~oes dos elementos de uma sequ\^encia finita $a|_{p+2}$ \'e inexor\'avel, seja qual for $a\in A^\omega$.
  \end{teo}
  \prova Primeiro, consideremos uma sequ\^encia $a\in A^\omega$ qualquer. Para $p=0$ temos $a_0\meet a_1$ \'e completamente n\~ao amb\'iguo. Agora, suponhamos que a propriedade seja v\'alida para todo $q\in p+1$. Estipularemos que fixados $k,l\in\omega$, tais que $k\in l+1\in p+2$, a associa\c c\~ao dos termos de uma sequ\^encia finita $a|_{l+1\cp k}$ seja denotado por
  \[
    \Meet_{j=k}^{l}a_j.
  \]
  Esta denota\c c\~ao \'e justificada pela nossa hip\'otese de indu\c c\~ao, pois $a|_{l+1\cp k}=b|_{l-k+1}$, com $b_m=a_{m+k}$, para todo $m\in l-k+1$, e todas as associa\c c\~oes com no m\'aximo $p+2$ termos s\~ao univocamente determinadas. 

  Uma associa\c c\~ao gen\'erica $\alpha$ de elementos da sequ\^encia $a|_{p+3}$, est\'a na imagem de $\meet$. Conformemente, existem $u,v\in A$, tais que $\alpha=u\meet v$. Nestas condições, $u$ \'e a associa\c c\~ao dos termos da sequ\^encia $a|_{i+1}$, e $v$ por sua vez, \'e a associa\c c\~ao dos termos da sequ\^encia $a|_{p+3\cp i+1}$, com $i+1\in p+2$. Esta \'utima estipula\c c\~ao a respeito de $i+1$, se deve ao fato de que $v$ deve ser composto de ao menos um operando. Seguidamente, conforme nossa hip\'otese de indu\c c\~ao, a associa\c c\~ao dos termos da sequ\^encia $a|_{i+1}$ \'e
  \[
    u=\Meet_{j=0}^ia_j
  \]
  e $a|_{p+3\cp i+1}=b|_{p+2-i}$, com $b_l=a_{i+1+l}$ para todo $l\in p+2-i$. Portanto, {\it mutatis mutandis}
  \[
    v=\Meet_{j=i+1}^{p+2}a_j.
  \]
  Por conseguinte,
  \[
    \begin{aligned}
      \alpha   & = u\meet v                                                                &                                     \cr
               & =\Meet_{j=0}^{i}a_j\meet\Meet_{j=i+1}^{p+2}a_j                            & (\text{hip\'otese de indu\c c\~ao}) \cr
               & =\Meet_{j=0}^{i}a_j\meet\biggl(\Meet_{j=i+1}^{p+1}a_j\meet a_{p+2}\biggr) & (\text{defini\c c\~ao})             \cr
               & =\biggl(\Meet_{j=0}^{i}a_j\meet\Meet_{j=i+1}^{p+1}a_j\biggr)\meet a_{p+2} & (\text{associatividade})            \cr
               & =\Meet_{j=0}^{p+1}a_j\meet a_{p+2}                                        & (\text{hip\'otese de indu\c c\~ao}) \cr
               & =\Meet_{j=0}^{p+2}a_j                                                     & (\text{defini\c c\~ao})             \cr
    \end{aligned}
  \]
  Consequentemente a propriedade \'e v\'alida para $p+1$, e a prova segue do princípio de indu\c c\~ao matemática.\qed

  % TODO:
  \begin{teo}%
    Seja $n_{p+1}$ o n\'umero de maneiras de escrever associa\c c\~oes de $p+1$ operandos com $p\in\omega$. Estipulando convenientemente que um elemento isolado \'e uma associa\c c\~ao. Ent\~ao , segue-se que para todo $p\in\omega$ vale
    \[
      n_{p+2}=\sum_{i=1}^{p+1}n_in_{p+2-i}.
    \]
  \end{teo}
  \prova Seja $p=0$, temos em conformidade
  \[
    n_2=n_1^2=\sum_{i=1}^1n_in_{2-i}
  \]
  pois cada operando de $\meet$ n\~ao se decomp\~oe em mais operandos, i.e., s\~ao isolados. Suponhamos por hip\'otese de indu\c c\~ao que a propriedade seja v\'alida para todo $k\in p+3$. Dados $p+3$ operandos a opera\c c\~ao mais externa \'e da forma $u\meet v$, em que $u$ \'e uma opera\c c\~ao de $i$ operandos e $v$ de $p+3-i$ operandos, com $i\in p+3\cp1$. Uma tal opera\c c\~ao tem $n_in_{p+3-i}$ associa\c c\~oes, $n_i$ do primeiro operando $u$ e $n_{p+3-i}$ do operando $v$, isto sucede da hip\'otese de indu\c c\~ao, porquanto $i,p+3-i\in p+3\cp1\subset p+3$. Como $i\in p+3\cp1$ 

  \qed

  A seguir apresentarei um teorema devido a Tarski segundo o qual podemos inferir o \rf{Teorema}{teo0801261705} como corol\'ario. Em verdade, trata-se de uma generaliza\c c\~ao do teorema do ponto fixo de Knaster-Tarski. Vale salientar que a prova do teorema \rf{Teorema}{teo0801261705} \'e autocontida, de maneira que n\~ao \'e preciso transcender o escopo para reticulados. Sobre essa perspectiva mais geral deve-se reconhecer os objetos do teorema anterior como uma realiza\c c\~ao do sistema de axiomas abarcados pelo \rf{Teorema}{teo1001262040}, ou seja, h\'a mais verbaliza\c c\~ao.

  \begin{teo}[Teorema do ponto fixo para reticulados\footnote{Enunciarei o teorema sob uma perspectiva mais geral seguindo o artigo de Tarski, que generalizou o teorema aplicado ao conjunto das partes de um conjunto arbitr\'ario para reticulados completos.}]%
    Sejam 
    \begin{enumerate}[label = ({\rm\roman*})]
      \item{%
        ${\frak X}=(X,\leq)$ um reticulado completo;
      }
      \item{%
        $f:X\ra X$ uma fun\c c\~ao crescente; 
      }
      \item{%
        $F$ o conjunto de todos pontos fixos de $f$.
      }
    \end{enumerate}
    Ent\~ao $F\neq\emptyset$ e o sistema ${\frak F}=(F,\leq)$ \'e um reticulado completo. Em particular, temos
    \[
      \Join F=\Join\{x:x\in X\,\a\,f(x)\leq x\}
    \]
    e
    \[
      \Meet F=\Meet\{x:x\in X\,\a\,x\leq f(x)\}.
    \]
    \label{teo1001262040}
  \end{teo}
  \prova Provar.\qed
  ...
  % TODO: Completar? Citar refer\^encia do artigo.
  \begin{teo}[Huntington]
    Seja $B$ um conjunto munido com uma opera\c c\~ao bin\'aria $\cup$ e uma opera\c c\~ao un\'aria $'$ satisfazendo os seguintes axiomas
    \medskip
    % TODO: Usar enumerate.
    \halign{
    ...
    \medskip
    Ent\~ao $(B,\cup,')$ \'e uma \'algebra de Boole.
  \end{teo}
  ...
  \begin{Def}
    Um conjunto ordenado $S$ \'e dito ter a propriedade da menor cota superior se, para todo conjunto n\~ao vazio $E$ limitado superiormente admite a menor das cotas superiores de $E$, denotada por $\sup E$.
  \end{Def}
  
  \begin{teo}
    Todo conjunto ordenado que tem a propriedade da menor cota superior tem a propriedade da maior cota superior.
  \end{teo}
  
  \prova Sejam $E\subseteq S$, n\~ao vazio limitado inferiormente e $L$ o cojunto das cotas inferiores de $E$ que, como sabemos \'e n\~ao vazio, pois $E$ \'e limitado inferiormente. Em vista da hip\'otese de $S$ ter a propriedade da menor cota superior existe $\sup L$. Seja agora $\gamma>\sup L$. Ent\~ao $\gamma$ n\~ao \'e cota inferior de $E$, pois do contr\'ario $\gamma\leq\sup L$, desta forma $\sup L$ \'e a maior das cotas inferiores de $E$, como consequencia $\inf E=\sup L$. Fica, portanto, completa a prova do teorema.\qed 
  
  % TODO: Melhorar.
  \begin{teo}[Expans\~oes $b$-\'adicas]
    Seja $\varrho\in\;]0,\infty[$, escolhamos $n_0\in N$, tal que
    $$
  ...
  \noindent{\bf Exerc\'\i cio 24 do livro {\it Calculus} de M. Spivak.\footnote{Existe uma maneira melhor de demonstrar isso usando indu\c c\~ao matem\'atica, veja Zariski ou Jacobson.}} Consideremos dada uma sequ\^encia de n\'umeros $\{a_n\}$, denotaremos uma soma qualquer dos $n\geq 1$ primeiros termos, aparacendo na ordem natural, por $s(a_1,\ldots,a_n)$, em outras palavras, $a_i$ vem antes de $a_{i+1}$ na express\~ao da soma. 
  ...
  \begin{exe}[Item (d) do exerc\'icio 11 do \pma]\rm%
    Esta parte deste exerc\'icio me deu sobremaneira trabalho. Supondo que $\{a_n\}$ seja uma sequ\^encia de n\'umeros reais, em $]0,\infty[$, tal que $\sum a_n=\infty$, o que se pode dizer sobre
  ...
  O objetivo das pr\'oximas se\c c\~oes \'e aplicar transforma\c c\~oes ao sistema de coordenadas, de tal maneira que a c\^onica seja facilmente reconhecida.

  % TODO: Completar e organizar esta seção.
  \subsection{Transla\c c\~oes de sistema de coordenadas.}

  % TODO: Produzir diagrama.
  \begin{figure}[!h]
    \centering
    \begin{tikzpicture}[
        >={
      ...
      \coordinate (O) at (0,0);
      \coordinate (Om) at (\h,\k);
      \coordinate (P) at (3.5,4);

      \fill (O)  circle [radius=1.5pt] node [anchor=north east] {$O$};
      \fill (Om) circle [radius=1.5pt] node [anchor=north east] {$\varOmega$};
      \fill (P)  circle [radius=1.5pt] node [anchor=south west] {$P$};
      

      \draw [->] (0,-\st) -- (0,5) node [above] {$y$};
      \draw [->] (-\st,0) -- (5,0) node [right] {$x$};

      \draw [dashed] (P) -- ($(O)!(P)!(5,0)$);
      \draw [dashed] (P) -- ($(O)!(P)!(0,5)$);

      \draw [dashed] (P) -- ($(O)!(P)!(5,0)$) coordinate (X) node [below] {$x$};
      \draw [dashed] (P) -- ($(O)!(P)!(0,5)$) coordinate (Y) node [left]  {$y$};

      \draw [dashed] (Om) -- ($(O)!(Om)!(5,0)$) coordinate (H) node [below] {$h$};
      \draw [dashed] (Om) -- ($(O)!(Om)!(0,5)$) coordinate (K) node [left]  {$k$};

      \draw [decorate, decoration={brace, amplitude=6pt}, line width=.6pt] (H) -- (X) node [midway, above=5pt] {$x-h$};
      \draw [decorate, decoration={brace, amplitude=6pt}, line width=.6pt] (Y) -- (K) node [midway, right=5pt] {$y-k$};

      \draw [->] (0+\h,-\st+\k) -- (0+\h,5) node [above] {$v$};
      \draw [->] (-\st+\h,0+\k) -- (5,0+\k) node [right] {$u$};

    \end{tikzpicture}
    \caption{Transla\c c\~ao dum sistema de coordenadas cartesiano.}
  \end{figure}


  Dados pontos $\varOmega=(h,k)$ e $P=(x,y)$ de $\R^2$, as coordenadas $(u,v)$ de $P$ relativo ao sistema cuja origem \'e $\varOmega$, s\~ao simplesmente dadas pela identidade
  \[
    (u,v)=(x,y)-(h,k).
  \]
  Mais precisamente a rela\c c\~ao entre os sistemas de coordenadas \'e dada pela transforma\c c\~ao afim $T:\R^2\ra\R^2$, definida por
  \[
    T(v)=v-\varOmega.
  \]
  ...
  A rela\c c\~ao das coordenadas de $(u,v)$ do sistema transladado com as coordenadas de $(x,y)$ segundo o sistema antigo pode ser sintetizada pelo sistema linear
  \[
    \Biggl\{\,
    \begin{aligned}
      x&=u+h\cr
  ...
  Em seguida, calculemos $G(u,v)=g(u+h,v+k)$. Conformemente,
  \[
    \begin{aligned}
      G(u,v)&=a(u+h)^2+b(u+h)(v+k)+c(v+k)^2+d(u+h)+e(v+h)+f\cr
  ...
  A seguir est\'a provado que no caso do sistema (\ref{S2912252034}) admitir infinitas solu\c c\~oes, que o termo independente de $g$ no novo sistema de coordenadas \'e inexor\'avel \`a escolha da solu\c c\~ao do sistema (\ref{S2912252034}).


  \begin{teo}[Exerc\'icio 23-10\;-\;\ct{BoulosCamargo}]
  ...
  Desenvolvendo (\ref{eq2912252055}) e tomando $(x,y)=(u,v)$ em (\ref{eq2912252123}) podemos inferir
  \[
    2(au^2+buv+cv^2+{d\over 2}u+{e\over 2}v)+2ahu+2ckv+bhv+buk=0,
    \label{eq2912252132}
  \]
  ou melhor,
  \[
    ...
    \label{eq1401261239}
  \]
  Como consequ\^encia, de (\ref{eq2912252053}), (\ref{eq2912252123}) e (\ref{eq1401261239}) temos
  \[
    \begin{split}
      g(h,k)&=ah^2+bhk+ck^2+dh+ck+f\cr
            &=ah^2+bhk+ck^2+dh+ck+f\cr
            &\qquad+\overbrace{au^2+buv+cv^2+{d\over 2}u+{e\over 2}v+2ahu+2ckv+bhv+buk}^{0\ (\ref{eq1401261239})}\cr
            &=\overbrace{a(h+u)^2+b(u+h)(v+k)+c(k+v)^2+{d\over2}(h+u)+{e\over2}(k+v)}^{0\ (\ref{eq2912252053})}\cr
            &\qquad+{d\over2}u+{e\over2}v+f\cr
            &={d\over2}u+{e\over2}v+f\cr
            &\kern-8pt\stackrel{(\ref{eq2912252123})}{=}g(u,v)
    \end{split}
  \]
  Em outros termos $g$ \'e constante no conjunto de solu\c c\~oes do sistema (\ref{S2912252034}).\qed
  

  % TODO: Demonstrar a rela\c c\~ao do sistema de coordenadas via geometria euclidiana.
...
%  \]
  \subsection{Rota\c c\~oes de sistemas de coordenadas}


  Consideremos um ponto (vetor) $P=(u,v)_\theta$ cujas coordenadas s\~ao dadas em rela\c c\~ao ao sistema rotacionado. Nosso objetivo \'e determinar as coordenadas de $P$ relativo ao primeiro sistema. Primeiramente usaremos o m\'etodo geom\'etrico e em seguida um alg\'ebrico.

  Observemos, pois, a figura seguinte
  ...
  \begin{figure}[!h]
    ...
    \begin{tikzpicture}[
      ...
      \def\rot((#1,#2),#3){
        ({#1*cos(#3)-#2*sin(#3)}, {#1*sin(#3)+#2*cos(#3)})
      }
      \def\st{.5}
      \def\sep{1.15}
      \def\ang{15}
      ...
      \coordinate (P) at (2.1, 3.5);
      \draw [fill=black] (P) circle [radius=1.5pt];
      \node at ($(O)!1.08!(P)$) {$P$};
      \draw [dashed] (P) -- (P|-O) coordinate (P1);
      \draw [dashed] (P) -- (P-|O) coordinate (P2);

      \draw [->] (0,-\st) -- (0,5) node [above] {$y$};
      \draw [->] (-\st,0) -- (5,0) node [right] {$x$};

      \draw [->] \rot((0, -\st), \ang) -- \rot((0, 5), \ang) coordinate (V) node [above] {$v$};
      \draw [->] \rot((-\st,0), \ang) -- \rot((5,0), \ang) coordinate (U)   node [right] {$u$};

      \draw [dashed] (P) -- ($(0,0)!(P)!(U)$) coordinate (P3);
      \draw [dashed] (P) -- ($(0,0)!(P)!(V)$) coordinate (P4);

      \foreach \i in {1,...,4} {
        \fill (P\i) circle [radius=1.5pt];
      }

      \draw (\radius,0) arc [start angle=0, end angle=\ang, radius=\radius];
      \node at (\ang/2:1.5) {$\theta$};
      \draw (0,\radius) arc [start angle=90, end angle=\ang+90, radius=\radius];
      \node at (\ang/2+90:1.5) {$\theta$};
      ...
      \node [left]  at (P4) {$P_4$};
    \end{tikzpicture}
    \caption{Rota\c c\~ao dum sistema de coordenadas cartesiano por um \^angulo $\theta$.}
  \end{figure}

  Seguirei a mesma linha de racioc\'inio empregada em \ct{Iezzi7}. Da geometria anal\'itica temos
  \[
      \ora{OP_1}=\ora{P_2P},\ \ora{OP_2}=\ora{P_1P},\ \ora{OP_3}=\ora{P_4P},\ \ora{OP_4}=\ora{P_3P}
  \]
  e
  \[
    \ora{OP}=\ora{OP_3}+\ora{P_3P}=\ora{OP_4}+\ora{P_4P}.
  \]

  Seguidamente, para $i\in\{1,2\}$, definamos
  \[  
    \pi_i(w)=\lge u,e_i\rge,
  \]
  as proeje\c c\~oes usuais sobre os eixos gerados pela base can\^onica $\{e_1,e_2\}$. Em conson\^ancia, temos
  \[
    \begin{aligned}
      \pi_1(\ora{OP})&=\pi_1(\ora{OP_3})+\pi_1(\ora{P_3P})\cr
                     &=\pi_1(\ora{OP_3})+\pi_1(\ora{OP_4})\cr
                     &=|\ora{OP_3}|\cos\theta+|\ora{OP_4}|\cos\Bigl({\pi\over2}+\theta\Bigr)\cr
                     &=|\ora{OP_3}|\cos\theta-|\ora{OP_4}|\sin\theta\cr
                     &=u\cos\theta-v\sin\theta\cr
    \end{aligned}
  \]
  e
  \[
    \begin{aligned}
      \pi_2(\ora{OP})&=\pi_2(\ora{OP_4})+\pi_2(\ora{P_4P})\cr
                     &=\pi_2(\ora{OP_4})+\pi_2(\ora{OP_3})\cr
                     &=|\ora{OP_4}|\cos\theta+|\ora{OP_3}|\cos\Bigl({\pi\over2}-\theta\Bigr)\cr
                     &=|\ora{OP_4}|\cos\theta+|\ora{OP_3}|\sin\theta\cr
                     &=v\cos\theta+u\sin\theta.\cr
    \end{aligned}
  \]

  Em s\'intese, obtemos a seguinte rela\c c\~ao das coordenadas do ponto $P$ no sistema anterior, em rela\c c\~ao \`as do novo sistema, nomeadamente
  \[
    \left\{\;
    \begin{aligned}
      x&=u\cos\theta-v\sin\theta\cr
      y&=u\sin\theta+v\cos\theta\cr
    \end{aligned}
    \right.
    \label{S0501262000}
  \]
  
  Uma outra maneira, \'e recorrer \`a \'algebra linear, mais precisamente a espa\c cos vetoriais munidos com produto interno. Seja ${\mathcal B}=\{e_1,e_2\}$ a base can\^onica do $\R^2$. Uma base ortonormal, para o segundo sistema \'e 
  \[
  ...
  e
  \[
    s=\cos\Bigl({\pi\over2}+\theta\Bigr)e_1+\sin\Bigl({\pi\over2}+\theta\Bigr)e_2=-\sin\theta e_1+\cos\theta e_2.
    \label{S0501262001}
  \]
  Al\'em do mais, o \^angulo entre $r$ e $e_1$ \'e 
  \[
    \eta=\arccos\Bigl({\lge r,e_1\rge\over |r||e_1|}\Bigr)=\arccos(\cos\theta).
  \]


  Portanto, um vetor $P=(u,v)_\theta$ relativo \`a base ${\mathcal B}_\theta=\{r,s\}$, tem coordenadas satisfazendo o sistema (\ref{S0501262000}). Assim, as coordenadas $x$ e $y$ de $P$ relativas ao sistema can\^onico satisfazem a identidade
  \[
    \begin{bmatrix}
      x\cr
      y\cr
  ...
  \`A transforma\c c\~ao linear $R_\theta:\R^2\ra\R^2$ cuja matriz associada \'e
  \[
    [R_\theta]=
    \begin{bmatrix}
      \cos\theta & -\sen\theta\cr
    ...
    \end{bmatrix}
  \]
  daremos o nome de rota\c c\~ao por um \^angulo $\theta$. Ademais, para qualquer $\theta$ t\^em-se
  \[
    [R_\theta]^{-1}=
    \begin{bmatrix}
       \cos\theta & \sen\theta\cr
    ...
    \end{bmatrix}.
  \]
  Em conson\^ancia, $R_\theta$ \'e um isomorfismo de $\R^2$.


  \subsection{Elimina\c c\~ao do termo quadr\'atico misto por rota\c c\~oes}

  Munido das observa\c c\~oes da se\c c\~ao anterior, apliquemos uma rota\c c\~ao ao nosso sistema de coordenadas. Fazendo a substitui\c c\~ao de $(x,y)$ segundo (\ref{eq3001261537}) obtemos
  \[
    \begin{aligned}
      G(u,v)&=(a\cos^2\theta+b\cos\theta\sin\theta+c\sin^2\theta)u^2+\bigl((a-c)\sin2\theta+b\cos2\theta\bigr)uv\cr
            &\qquad+(a\sin^2\theta-b\cos\theta\sin\theta+c\cos^2\theta)v^2+(d\cos\theta+e\sin\theta)u\cr
            &\qquad+(e\cos\theta-d\sin\theta)v+f\cr
    \end{aligned}
  \]
  Por quest\~oes pr\'aticas, escreveremos
  \[
    G(u,v)=\alpha u^2+\beta uv+\gamma v^2+\delta u+\varepsilon v+f,
  \]
  o que pressup\~oe as igualdades
  \[
    \begin{aligned}
      \alpha      &=a\cos^2\theta+b\cos\theta\sin\theta+c\sin^2\theta\cr
      \beta       &=(a-c)\sin2\theta+b\cos2\theta\cr
      \gamma      &=a\sin^2\theta-b\cos\theta\sin\theta+c\cos^2\theta\cr
      \delta      &=d\cos\theta+e\sin\theta\cr
      \varepsilon &=e\cos\theta-d\sin\theta\cr
    \end{aligned}
    \label{eq0301261557}
  \]
  Agora, nosso intento \'e que o termo misto se anule, para este fim devemos determinar $\theta$, tal que $\beta=0$.  Notavelmente, estamos estipulando {\it a priori} que $b\neq0$, pois do contr\'ario a rota\c c\~ao do sistema de coordenadas n\~ao teria des\'ignio algum. Em conformidade com esta estipula\c c\~ao e com as nota\c c\~oes anteriores segue-se o 
  \begin{teo}
    Seja $C$ uma c\^onica cujo o termo quadr\'atico misto do polin\^omio subjacente $g$ seja n\~ao nulo. Ent\~ao existe uma rota\c c\~ao $R_\theta$ de um \^angulo $\theta$, segundo a qual o termo quadr\'atico misto do polin\^omio subjacente $G=gR_\theta$ \'e nulo
    \label{teo0301261705}
  \end{teo}
  \prova De (\ref{eq0301261557}) segue que 
  \[
    (a-c)\sin2\theta=b\cos2\theta
    \label{eq0301261559}
  \]
  Da\'i vem necessariamente que $2\theta\neq n\pi$, para todo $n\in\Z$, consequentemente $\sin2\theta\neq0$. Com efeito, suponha que exista $n\in\Z$, tal que $2\theta=n\pi$, decorre de (\ref{eq0301261559}) que
  \[
    b=b\cos2\theta=(a-c)\sin2\theta=0,
  \]
  o que contradiz nossa suposi\c c\~ao. Portanto, podemos escrever
  \[
    \cot2\theta={\cos2\theta\over\sin2\theta}={a-c\over b}.
    \label{eq0301261607}
  \]
  Temos duas solu\c c\~oes para (\ref{eq0301261607}) em $[0,2\pi[$ uma no semiciclo superior e outra no inferior.\qed

  Escolhido $\theta$ conforme no \rf{Teorema}{teo0301261705}, temos de (\ref{eq0301261557}) e (\ref{eq0301261607}) que
  \[
    \alpha-\gamma=b\cot2\theta\cos2\theta+b\sin\theta={b\over\sin2\theta}.
  \]
  Em conson\^ancia com (\ref{eq0301261557}), podemos inferir
  \[
    \alpha={1\over2}\Bigl(a+c+{b\over\sin2\theta}\Bigr)\quad\hbox{e}\quad\gamma={1\over2}\Bigl(a+c-{b\over\sin2\theta}\Bigr)
  \]
  As identidades trigonom\'etricas 
  \[
    \begin{aligned}
    \sin^22\theta &={1\over1+\cot^22\theta}\cr
    \cos2\theta   &=\cot2\theta\sin2\theta \cr
    \cos^2\theta  &={1+\cos2\theta\over2}  \cr
    \sin^2\theta  &={1-\cos2\theta\over2}  \cr
    \end{aligned}
  \]
  nos permitem determinar uma solu\c c\~ao do sistema
  \[
    \left\{\;
    \begin{aligned}
      \delta      &=d\cos\theta+e\sin\theta\cr
  ...
  Em suma, o que podemos concluir \'e que dada uma c\^onica, podemos sempre supor sem perda de generalidade que o termo quadr\'atico misto do polin\^omio subjacente seja nulo.

  \begin{obs}\rm%
    Relembremos que dada uma fun\c c\~ao polinomial do segundo grau $p:\R\ra\R$, cuja lei \'e
    \[
      p(\lambda)=\lambda^2+b\lambda+c.
    \]
    Vale
    \[
      r_1+r_2=-b\quad\hbox{e}\quad r_1r_2=c,
    \]
    em que $r_i$ com $i=1,2$, s\~ao as ra\'izes da equa\c c\~ao $p(\lambda)=0$.

    Nesta se\c c\~ao, escolhemos um $\theta$ adequado para que $\beta=0$, com isso determinamos
    \[
      \alpha={1\over2}\Bigl(a+c+{b\over\sin2\theta}\Bigr)\quad\hbox{e}\quad\gamma={1\over2}\Bigl(a+c-{b\over\sin2\theta}\Bigr).
    \]
    Da\'i temos
    \[
      \begin{aligned}
       \alpha\gamma&={1\over4}\biggl((a+c)^2-{b^2\over\sin^22\theta}\biggr)\cr
                   &={(a+c)^2\sin^22\theta-b^2\over4\sin^22\theta}\cr
                   &={(a+c)^2\sin^22\theta-b^2(\cos^22\theta+\sin^22\theta)\over4\sin^22\theta}\cr
                   &={(a+c)^2\sin^22\theta-\overbrace{b^2\cos^22\theta}^{(\ref{eq0301261559})\ (a-c)^2\sin^22\theta}-b^2\sin^22\theta\over4\sin^22\theta}\cr
                   &={(a+c)^2\sin^22\theta-(a-c)^2\sin^22\theta-b^2\sin^22\theta\over4\sin^22\theta}\cr
                   &={\sin^22\theta\bigl((a+c)^2-(a-c)^2-b^2\bigr)\over4\sin^22\theta}\cr
                   &={(a+c)^2-(a-c)^2-b^2\over4}\cr
                   &={(a+c+a-c)(a+c-(a-c))-b^2\over4}\cr
                   &={4ac-b^2\over4}\cr
                   &=ac-{b^2\over4}.\cr
      \end{aligned}
    \]
    Adicionamente, sabendo que $\alpha+\gamma=a+c$, segue-se que $\alpha$ e $\gamma$ s\~ao ra\'izes do polin\^omio $p$, dado por
    \[ 
      p(\lambda)=\lambda^2-(a+c)\lambda+ac-{b^2\over4}.
    \]
    Logo,
    \[
      p(\lambda)=0\;\,\hbox{se, e somente se,}\;\,\det([M_p-\lambda I])=0
    \]
    em que
    \[
      [M_p]=
      ...
      \end{bmatrix},
    \]
    i.e., $\alpha$ e $\gamma$ s\~ao os autovalores da transforma\c c\~ao $M_p$.
  ...
  \begin{thebibliography}{}
    \bibitem{BoulosCamargo}
    CAMARGO, IVAN DE; BOULOS, PAULO. {\it Geometria Anal\'itica}. 3ª ed. rev. e ampl. S\~ao~Paulo: Prentice Hall, 2005.
    \bibitem{HalmosNaive}
    HALMOS, PAUL R. {\it Naive Set Theory}. Garden City, New York: Dover Publications, 2017.
    ...
    \bibitem{Iezzi7}
    IEZZI, GELSON. {\it Fundamentos de Matem\'atica Elementar}. Vol. 7. Geometria Anal\'itica. 3ª ed. S\~ao Paulo: Atual, 1985.
    \bibitem{RudinPrinciples}
    RUDIN, WALTER. {\it Principles of Mathematical Analysis}. $3^\text{rd}$ ed. McGraw-Hill, 1976.
    \bibitem{SpivakCalculus}
    SPIVAK, MICHAEL. {\it Calculus}. $4^\text{th}$ ed. Houston, Texas: Publish or Perish, 2008.
  \end{thebibliography}

\enddoc

% TODO: Geometria euclidiana;
% TODO: Geometria anal\'itica;
% TODO: \'Algebra Linear;
% TODO: Anel dos inteiros;
% TODO: MMC e MDC;
% TODO: Anel dos polin\^omios;
% TODO: Anel das matrizes;
% TODO: Espa\c cos vetoriais;
% TODO: M\'odulos;
% TODO: Regra de tr\^es simples e composta;
% TODO: Inconsist\^encia de modelos LLM;
% TODO: Diferencibilidade de fun\c c\~oes reais inversas de vari\'aveis reais;
% TODO: Mudan\c ca de vari\'aveis em integrais;
% TODO: Defini\c c\~ao de primitivas ou integrais indefinidas.
