...
\begdoc
  ...
  \begin{teo}[Associatividade generalizada]%
    Sejam $A$, $\meet$ \rf{Defini\c c\~ao}{def2101262009}. Suponha adicionamente que $\meet$ seja associativa. Ent\~ao para cada $p\in\omega$ o resultado de associa\c c\~oes dos elementos de uma sequ\^encia finita $a|_{p+2}$ \'e inexor\'avel, seja qual for $a\in A^\omega$.
  \end{teo}
  \prova Para $p=0$ temos $a_0\meet a_1$ \'e completamente n\~ao amb\'iguo. Agora, suponhamos que a propriedade seja v\'alida para todo $i\in p+1$. Estipularemos que fixado $k\in\omega$ o produto dos termos de uma sequ\^encia finita $a|_{k+2\cp k}$ seja denotado por
  \[
    \Meet_{j=k}^{k+1}a_j.
  \]
  Um produto gen\'erico de elementos da sequ\^encia $a|_{p+3}$ \'e da forma $u\meet v$, com $u$ composto de $i+1\in p+3\cp1$ operandos e $v$ composto de $p+2-i\in p+3\cp1$, operandos, temos portanto
  \[
    \begin{aligned}
      u\meet v & =\Meet_{j=0}^{i}a_j\meet\Meet_{j=i+1}^{p+2}a_j                            & (\text{hip\'otese de indu\c c\~ao}) \cr
