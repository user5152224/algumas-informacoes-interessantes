...
\begin{teo}[4.31 Remark: {\it Principles of Mathematical Analysis}]
  ...
  \]
  e 
  $\alpha_x\in(\kappa_x)^\omega$ uma fun\c c\~ao qualquer satisfazendo a condi\c c\~ao: se $\kappa_x$ for finito, ent\~ao existe $n_x\in\omega$, tal que $\alpha_x(n_x)=\varkappa_x$ e $\alpha_x(\cp n_x)=\{0\}$. Ent\~ao
  \[
    \Sigma_x=\sum_{k=0}^\infty\alpha_{xk}<\infty.
  \]
  E o n\'umero $\Sigma_x$ independe da fun\c c\~ao $\alpha_x$.

  Ent\~ao $f\in\R^{]a,b[}$, tal que $f(x)=\Sigma_x$ tem as seguintes propriedades:
  \begin{enumerate}[label = (\alph*)]
    \item{%
  ...
  \end{enumerate}
\end{teo}
\prova Primeiramente, suponha que $a<x<y<b$. Sabe-se que $\varkappa_x\subset\varkappa_y$. Temos duas possibilidades a considerar $\varkappa_y$ \'e finito ou infinito. Se $\varkappa_y$ for finito, ent\~ao $\varkappa_x$ e
  \[
    \delta=\varkappa_y\cp\varkappa_x,
  \]
  tamb\'em o \'e ser\~ao. Logo $f(x)\leq f(y)$, segue da manipula\c c\~ao de somas finitas. 

  Agora, suponhamos que $\varkappa_y$ seja infinito. Se $\delta$ for finito, podemos reordenar $\sum_{k\in\varkappa_y}a_k$ de tal maneira que os seus primeiros termos coincidam com os de $\delta$. Caso $\delta$
