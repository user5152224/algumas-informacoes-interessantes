%!latex
\documentclass[10pt, a5paper, leqno]{book}
...
Os cortes de Dedekind nos fornece um caminho para a constru\c c\~ao de um corpo ordenado $R$ com a propriedade da menor cota superior, existem certamente outros caminhos como usando as sequ\^encias de Cauchy, resultado devido a Cantor. Surgem as perguntas: O que individualiza o conjunto $R$? Ser\'a que este conjunto assim constru\'\i do \'e de fato o conjunto n\'umerico conhecido desde a escola prim\'aria? As constru\c c\~oes mencionadas n\~ao resultam em dois objetos matem\'aticos aparentemente distintos? Adianto que o conjunto dos n\'umeros reais conhecidos na escola s\~ao na realidade s\'eries num\'ericas, ao passo que aquele que constru\'\i mos \'e uma cole\c c\~ao de subconjuntos de n\'umeros racionais. Como bem observado em [Elon], intrinsecamente estes objetos s\~ao distintos, mas do ponto de vista de isomorfismos s\~ao id\^enticos. Para tratar bem do caso deve-se concluir o exerc\'\i cio 2 do cap\'\i tulo 29 de [Spivak].  A seguir empenhar-nos-emos a provar que $R$ \'e \'unico a menos de isomorfismos, para tanto precisaremos de algumas constru\c c\~oes auxiliares. 
...
\begin{enumerate}[label = (\alph*)]
  ...
  \item{%
    Seja $m\in\omega$. Consideremos o conjunto
    \[
  ...
  \item{%
    Sejam $x,y\in K$, da mesma maneira seja
    \[
  ...
  \item{%
    Seja $m\in\omega$. Consideremos novamente
    \[
    ...
    Se $n\in S$, ent\~ao
    \[ 
      m*\bigl((n+1)*x\bigr)=m*\bigl(n*x+x\bigr)=mn*x+m*x=(mn+m)*x=(m(n+1))*x
    \]
  }
  \item{%
  }
...
\lemma{}{\sl%
  ...
  \vbox{
    \halign{
      \hfill# & #\hfill \cr
      {\rm a)} & $m*x+n*x=(n+m)*x;$        \cr
      {\rm b)} & $n*x+n*y=n*(x+y);$        \cr
      {\rm c)} & $n*(m*x)=nm*x;$           \cr
      {\rm d)} & $(n*x)\cdot y=n*xy;$      \cr
      {\rm e)} & $(m*x)\cdot (n*y)=mn*xy;$ \cr
    }
  }
  \medskip
}

\proof a) Fixemos $m\in N$ e consideremos o conjunto
$$
  S=\bigl\{n\in N:m*x+n*x=(n+m)*x\bigr\}.
$$
Certamente,
$$
  m*x+0*x=m*x=(m+0)*x,
$$
i.e., $0\in S$. Supondo que $n\in S$, temos que
$$
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
      $m*x+(n+1)*x$ & $=m*x+n*x+x$\cr
      \omit         & $=(m+n)*x+x$  \cr
      \omit         & $=\bigl(m+(n+1)\bigr)*x$ \cr
    }
  }
$$
ou seja $n+1\in S$. Provamos portanto que $S\subset N$ \'e um conjunto sucessor, mas como $ N$ \'e um conjunto sucessor minimal conclu\'\i mos que $S= N$. Por fim, da natureza arbitr\'aria de $m\in N$, podemos concluir a identidade.

b) Evidentemente 
$$
  0*x+0*y=0_K=0*(x+y).
$$
Suponhamos por hip\'otese de indu\c c\~ao que a propriedade seja v\'alida para $n$, seguidamente notemos que
$$
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
...
$$
portanto a propriedade \'e v\'alida para $n+1$ e, pelo princ\'\i pio de indu\c c\~ao matem\'atica ser\'a v\'alida para todo $n\in N$. 

c) Analogamente fixemos $m\in N$, e consideremos o conjunto
$$
  S=\{n\in N:n*(m*x)=nm*x\}.
$$
Em verdade, \'e f\'acil ver que $0\in S$. Suponhamos que $n\in S$, notemos que
$$
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
      $(n+1)*(m*x)$ & $=n*(m*x)+m*x$ \cr
      \omit         & $=nm*x+m*x$    \cr
      \omit         & $=\bigl((n+1)m\bigr)*x,$    \cr
    }
  }
$$
i.e., $n+1\in S$. Pelo mesmo motivo $S= N$ e da arbitrariedade de $m$ fica provada a identidade.

d) Inteiramente an\'alogo aos itens a) e b).

e) \'E suficiente notar que
$$
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
      $(m*x)\cdot(n*y)$ & $=m*\bigl(x\cdot(n*y)\bigr)$ \cr
      \omit             & $=m*(n*xy)$             \cr
      \omit             & $=mn*xy$                \cr
    }
  }
...
\begin{Def}
  ...
  \[
    ...
    \end{cases}
  \]
  Al\'em disso, para $m\in Z$ definamos
  \[
    m^*=|m|*\sigma(m)
  \]
  e
...
\begin{Def}
  Seja $K$ um corpo com unidade $1_k$ e zero $0_k$. Definimos a caracter\'istica de $K$ como 
  \[
    X(K)=\min\{n:n\in\omega\a n^*=0_k\}.
  \]
  \label{def_X}
\end{Def}
...
\begin{teo}
  Seja $K$ um corpo tal que $X(K)=0$. Ent\~ao existe um $Z^*\subset K$, tal que $Z$ \'e isomorfo a $Z$.
\end{teo}

\prova De posse da defini\c c\~ao {\bf \ref{def_int_K}}, podemos definir uma fun\c c\~ao $\zeta:Z\lra K$ dado por
\[
  \zeta(m)=m^*.
\]
Doravante, nosso objetvo limitar-se-a \`a prova de que $\zeta$ \'e um isomorfismo.
...
\lemma{}{\sl%
  ...
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
      {\rm a)} & $\forall n \bigl(n\in N\longrightarrow n*(-1_K)+1_K=(n-1)*(-1_K)\bigr);$ \cr
      {\rm b)} & $\varsigma(mn)=\varsigma(m)\cdot\varsigma(n);$                           \cr
      {\rm c)} & $(-n)^*=-n^*;$                                                           \cr
      {\rm d)} & $n^*+1^*=(n+1)^*;$                                                       \cr
      {\rm e)} & $\forall n \bigl(n\in N\longrightarrow n^*\geq0^*\bigr);$ \cr
      {\rm f)} & $\forall m\bigl(m\in N\longrightarrow n^*+m^*=(n+m)^*\bigr);$            \cr
      {\rm g)} & $(n+m)^*=n^*+m^*;$                                                       \cr
      {\rm h)} & $m^*\cdot n^*=(mn)^*;$                                                    \cr
      {\rm i)} & $m<n\longrightarrow m^*<n^*.$                                            \cr
    }
  }
}

\proof 
a) Primeiro, observe que
$$
  0*(-1_K)+1_K=1_K=(-1)*(-1_K)=(0-1)*(-1_K),
$$
i.e., a identidade \'e v\'alida para $n=0$. Suponhamos por hip\'otese de indu\c c\~ao que ela o seja para $n$, em seguida observemos
$$
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
      $(n+1)*(-1_K)+1_K$ & $=n*(-1_K)-1_K+1_K$              \cr
      \omit              & $=n*(-1_K)$                   \cr 
      \omit              & $=\bigl((n+1)-1\bigr)*(-1_K),$ \cr
    }
  }
$$
portanto, \'e v\'alida para $n+1$, conclu\'\i mos do princ\'\i pio de indu\c c\~ao mate\-m\'atica que a propriedade \'e v\'alida para todo $n\in N$.


b) A fun\c c\~ao $\varsigma$ \'e a fun\c c\~ao sinal, verifica-se facilmente, considerando casos, que ela cumpre as propriedades do sinal, ou seja, o sinal do produto \'e o produto dos sinais.
...
$$
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
      $(-n)^*$ & $=|-n|*\varsigma(-n)$                             \cr
      \omit    & $=|n|*\bigl(\varsigma(-1)\cdot\varsigma(n)\bigr)$ \cr
      \omit    & $=|n|*\bigl(-1_K\cdot\varsigma(n)\bigr)$          \cr
      \omit    & $=-1_K\cdot\bigl(|n|*\varsigma(n)\bigr)$          \cr
      \omit    & $=-n^*$                                           \cr
    }
  }
$$

d) Se $n\in N$, a identidade \'e trivialmente v\'alida, pois segue da defini\c c\~ao. Se $n\in Z\setminus N$, ent\~ao temos dois casos a considerar $n=-1$ ou $n<-1$. Suponha que $n=-1$, conformemente temos
$$
  (-1)^*+1^*=-1^*+1^*=0_K=0^*=(-1+1)^*.
$$
Agora admita que $n<-1$, temos em conformidade
$$
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
      $n^*+1^*$ & $=(-n)*(-1_K)+1_K$      \cr
      \omit     & $=(-n-1)*(-1_K)$        \cr
      \omit     & $=|n+1|*\varsigma(n+1)$ \cr
      \omit     & $=(n+1)^*.$             \cr
    }
  }
$$

e) Provemos por indu\c c\~ao sobre $n$. Observemos inicialmente que trivialmente $0^*\geq 0^*$, portanto vale para $n=0$. Suponhamos por hip\'otese de indu\c c\~ao que $n^*\geq0^*$, temos 
$$
  (n+1)^*=n^*+1^*>0,
$$
pois $1^*>0$, o que prova que a desigualdade \'e v\'alida para $n+1$, concluindo por pelo princ\'\i pio de indu\c c\~ao matem\'atica que ela \'e v\'alida para todo $n\in N$.


f) Provemos usando o princ\'\i pio de indu\c c\~ao matem\'atica. Primeiramente notemos que
$$
  n^*+0^*=n^*=(n+0)^*,
$$
logo, a propriedade \'e v\'alida para $m=0$. Suponhamos que tamb\'em seja para $m$, temos posteriormente
$$
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
      ...
      \omit         & $=(n^*+m^*)+1^*$          \cr
      \omit         & $=(n+m)^*+1^*$            \cr
      \omit         & $=\bigl((n+m)+1\bigr)^*$  \cr
      \omit         & $=\bigl(n+(m+1)\bigr)^*,$ \cr
    }
  }
$$
i.e., a propriedade \'e v\'alida para $m+1$, consequentemente pelo princ\'\i pio de indu\c c\~ao matem\'atica, conclu\'\i mos o requerido.

g) \'E suficiente notar que se $m<0$, ent\~ao $-m\in N$, da\'\i\ 
$$
  (n+m)^*-m^*=(n+m)^*+(-m)^*=n^*
$$
donde
...
$$
  \vbox{
    \halign{
      \hfill#&#\hfill\cr
      $m^*\cdot n^*$ & $=\bigl(|m|*\varsigma(m)\bigr)\cdot\bigl(|n|*\varsigma(n)\bigr)$ \cr
      \omit          & $=|m||n|*\bigl(\varsigma(m)\cdot\varsigma(n)\bigr)$              \cr
      \omit          & $=|mn|*\varsigma(mn)$                                            \cr
      \omit          & $=(mn)^*$                                                        \cr
    }
  }
$$

i) Notemos primeiramente que se $n>0$, ent\~ao existe $k\in N$, tal que $n=k+1$, da\'\i\ $n^*=k^*+1^*>0^*$. Logo, se $n>m$, ent\~ao $(n-m)^*=n^*-m^*>0^*$, consequentemente $n^*>m^*$.
\qed
...
%Provemos este item considerando casos. Primeiro observemos que se $m=0$, ent\~ao $n>0$, da\'\i\
%$$
%  n^*=n*1_K>0=0^*.
%$$
%
%Igualmente se $n=0$, ent\~ao $m<0$, logo, $-m>0$, e consequentemente $(-m)^*>0^*$, portanto $m^*<0^*$, pelo item b).
%
%Suponhamos que $m,n\neq0$, consideremos primeiramente que $0<m<n$. Neste caso temos 
%$$
%  m^*<m^*+(n-m)^*=n^*,
%$$
%pois $m,n-m\in N$. Seguidamente, suponhamos que $n>0>m$, neste caso segue trivialmente do primeiro caso que
%$$
%  m^*<0^*<n^*.
%$$
%Por fim, suponhamos que $n<m<0$, ent\~ao $0<-m<-n$, logo 
%$$
%  -m^*=(-m)^*<(-n)^*=-n^* 
%$$
%donde vem $n^*<m^*$. Os casos tratados s\~ao suficientes para concluir a implica\c c\~ao requerida. 
...
Neste ponto resolvi compilar alguns resultados b\'asicos do c\'alculo (ou an\'alise em $R$) para refer\^encias futuras.
...
Nesta se\c c\~ao inclu\'i alguns resultados matem\'aticos que auxiliam na prova de determinados resultados.

\noindent{\bf Lema.\ }{\sl Seja $f$ uma fun\c c\~ao tal que $f(t)\neq0$, para todo $t\in\hbox{\rm dom}\,f$. Ent\~ao}
$$
  {d\over dt}\Bigl(\ln\bigl|f(t)\bigr|\Bigr)={d f\over dt}(t){1\over f(t)}.
$$

\proof Primeiro relembremos da fun\c c\~ao sinal, a saber,
$$
  \sigma(x)=(\chi_{R^*_+}-\chi_{R^*_-})(x)
$$
em que $\chi_A$ \'e a fun\c c\~ao caracter\'\i stica de A. \'E deveras f\'acil ver que
$$
  {d\over dx}|\,|(x)=\sigma(x)|_{R^*}
$$
ou simplesmente
$$
  {d\over dx}|\,|(x)=\sigma(x)
$$
quando o dom\'\i nio \'e sabido, em que $|\,|$ \'e a fun\c c\~ao modular ou m\'odulo. 

Tamb\'em n\~ao \'e dif\'\i cil ver que para todo $x\in R$
$$ 
  |x|=\sigma(x)\cdot x
$$
...
$$
  {d\over dt}\Bigl(\ln\bigl|f(t)\bigr|\Bigr)={1\over\bigl|f(t)\bigr|}\cdot\sigma\bigl(f(t)\bigr)\cdot{df\over dt}(t)={d f\over dt}(t){1\over f(t)}.
$$
\qed
